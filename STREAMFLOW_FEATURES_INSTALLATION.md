# StreamFlow Features Installation für ECM

## ✅ Was wurde portiert?

Folgende Features aus StreamFlow-mod wurden erfolgreich nach ECM portiert:
...
- **Datei:** `ECM/backend/streamflow_features_config.py`
...
├── streamflow_features_config.py      # Konfigurations-Management
└── routers/
    └── streamflow_features.py         # API-Endpunkte
```

### Dokumentation:
```
ECM/
├── STREAMFLOW_FEATURES_README.md      # Feature-Dokumentation
└── STREAMFLOW_FEATURES_INSTALLATION.md # Diese Datei
```

### Konfiguration:
```
/config/streamflow_features.json       # Wird automatisch erstellt
```
...
curl http://localhost:8000/api/streamflow-features/config

# Provider Diversification aktivieren
curl -X PUT http://localhost:8000/api/streamflow-features/provider-diversification \
  -H "Content-Type: application/json" \
  -d '{"enabled": true, "mode": "round_robin"}'

# Account Stream Limits aktivieren
curl -X PUT http://localhost:8000/api/streamflow-features/account-stream-limits \
  -H "Content-Type: application/json" \
...
Öffne http://localhost:8000/api/docs und navigiere zu "StreamFlow Features"
...
curl -X PUT http://localhost:8000/api/streamflow-features/config \
  -H "Content-Type: application/json" \
  -d '{
...
Bearbeite `/config/streamflow_features.json`:
...
from streamflow_features_config import get_streamflow_features_config

# In _smart_sort_streams oder nach dem Sorting:
config = get_streamflow_features_config()
...
|----------|---------|--------------|
| `/api/streamflow-features/config` | GET | Komplette Konfiguration abrufen |
| `/api/streamflow-features/config` | PUT | Konfiguration aktualisieren |
| `/api/streamflow-features/config/reset` | POST | Auf Defaults zurücksetzen |
| `/api/streamflow-features/provider-diversification` | GET | Provider Diversification Config |
| `/api/streamflow-features/provider-diversification` | PUT | Provider Diversification Update |
| `/api/streamflow-features/account-stream-limits` | GET | Account Stream Limits Config |
| `/api/streamflow-features/account-stream-limits` | PUT | Account Stream Limits Update |
| `/api/streamflow-features/m3u-priority` | GET | M3U Priority Config |
| `/api/streamflow-features/m3u-priority` | PUT | M3U Priority Update |
...
# Prüfen
curl http://localhost:8000/api/streamflow-features/provider-diversification
```
...
# Prüfen
curl http://localhost:8000/api/streamflow-features/account-stream-limits
```
...
```bash
# Konfigurieren
curl -X PUT http://localhost:8000/api/streamflow-features/m3u-priority \
  -H "Content-Type: application/json" \
  -d '{
    "mode": "all_streams",
    "account_priorities": {
...
# Prüfen
curl http://localhost:8000/api/streamflow-features/m3u-priority
```
...
```
[STREAMFLOW-CONFIG] Configuration loaded successfully
...
Folgende StreamFlow-Features wurden **NICHT** portiert, da sie FFmpeg Stream Analysis benötigen:
...
- ✅ Initiale Portierung aus StreamFlow-mod
...
2. Prüfe die Konfiguration: `cat /config/streamflow_features.json`
3. Teste die API: `curl http://localhost:8000/api/streamflow-features/config`
...
- [STREAMFLOW_FEATURES_README.md](STREAMFLOW_FEATURES_README.md) - Detaillierte Feature-Dokumentation

...
account's group settings, or that is the target of a [Channel Group
Override](#using-a-dispatcharr-channel-group-override) on such a group (the
pre-flight reads the target's auto-sync state through its source). That is
an ownership constraint, not a lookup detail. Dispatcharr owns the master
channels' lifecycle (it creates, updates and deletes them from the
auto-synced group), and ECM only ever attaches streams to channels that
already exist there. A Dispatcharr channel group you built by hand, with
no M3U group feeding it directly or through an override, is therefore
**not supported as a master**; the pre-flight reports it under
`group_settings_found` and says so. When the master is scoped to a specific
provider, the pre-flight checks only that provider's row for the group, and
a miss there is reported as a missing provider/group association (with a
note if another account does carry the group), not as an unbacked group.
...
* every configured group still exists in some account's group settings
  (`group_settings_found`), directly or, for a whole-group scope, as the
  target of a [Channel Group
  Override](#using-a-dispatcharr-channel-group-override) on an auto-synced
  group. For a whole-group **master** this is the M3U-backed ownership
  constraint described in [Pick the master group](#1-pick-the-master-group):
  a hand-curated channel group is not supported as a master, and the
  failure message says so rather than reporting a missing group. For a
  **provider-scoped master** the check consults only the selected
  provider's row, so the message names that provider/group association
  and says whether another account carries the group. For a **secondary**
  it means the provider group was removed or renamed, or that provider no
  longer carries it.

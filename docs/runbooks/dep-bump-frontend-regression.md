...
- [ ] Re-cut process: once root-caused, re-propose the frontend bump. Follow ADR-001 cadence (one major per PR), run the [dep-bump fresh-image smoke test](./dep-bump-smoke-test.md) locally, and run through the app manually (drag-reorder, channel list, EPG, settings) before merging. PR6 (Vite 8) was sequenced after PR5 (React 19 ecosystem) deliberately — if PR5 is the regression, PR6 should not move until PR5 is resolved.

...
| Logos | Any non-zero count | **A logo miss is currently fatal to the whole restore, not just a warning.** As of `0.18.1-0023`, the backup does not archive the bytes of logos uploaded through ECM's own Logo Manager, even though Dispatcharr serves them on request (`enhancedchannelmanager-xb58a`). Because those bytes were never captured, a logo originally uploaded through ECM's own Logo Manager makes the restore abort and roll back entirely (`enhancedchannelmanager-d0agi`). Removing the ECM-uploaded logo record before backing up is the only way to get the restore to complete today; the logo itself is then not preserved. The preview also can't be trusted for this category specifically: it may report logo failures that don't occur on apply, or miss the one that will (`enhancedchannelmanager-dgnms`). |
...
**6.6** If logos are missing (red banner on restore-complete), and the restore still reached this phase, it's a warning: channels work. Re-upload logos manually or re-run an EPG logo match. If instead the restore never completed and reported `partial_failed_rolled_back` at the logo category, that's the fatal case above (`enhancedchannelmanager-d0agi`), not this one: the whole restore rolled back, not just the logos.
...
4. **Do not click Apply** as part of this exercise — a monthly maintenance check is a preview only, against a real artifact, on your real instance. (If you do want to prove the apply path end-to-end, do it against a disposable/throwaway Dispatcharr instance, never the production one — see `tests/dbas-test-env/` for the project's own disposable-instance tooling, or follow [Run a restore drill](../user_guide/backup-restore/run-a-restore-drill.md) for a full compose-based procedure, written to be sufficient without repository access, that also documents the current known failures for the apply path, including a P0 where the backup does not archive ECM-uploaded logo bytes, aborting the restore if the source has any.)

...
- **Artifact handling after egress** is unchanged in kind but materially smaller in degree. A standard artifact no longer carries working credentials or ECM accounts, but it is not categorically safe for public disclosure: channel and group names, rule definitions and notes, and credential-free provider addresses remain. The operator must assess that authored content before attaching the artifact to a support ticket.

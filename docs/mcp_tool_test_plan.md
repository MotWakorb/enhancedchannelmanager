...
> test plan covering **every one of the 116 tools** the ECM MCP server exposes,
...
3. **Use real IDs.** Many tools need an id (channel/group/account/rule/etc.).
   Run the matching `list_*` / `get_*` tool first and feed a real id into the
   next prompt. The prompts below say when to do this.
...
## 🔥 Suspected issues — test these first (0.17.2 fix candidates)

These came out of a code read of every tool. They are **hypotheses to confirm**,
not verified bugs — but they're the most likely culprits for "tools that don't
work the way I'd hope," so start here. Each maps to a tool entry below.
...
Mark each as you go: ✅ pass · ❌ fail · ⚠️ partial. (116 tools)
...
- **Stats & Analytics (7):** get_channel_stats · get_top_watched · get_bandwidth · get_popularity_rankings · get_watch_history · get_unique_viewers · compute_stream_sort
...
- **Export & Publish (6):** list_export_profiles · generate_export · create_export_profile · delete_export_profile · list_cloud_targets · publish_export
...
- **Edge / failure tests:**
  - "List channels in group 99999 (non-existent group)" → Should return "No channels found" (empty list from backend), not an error crash.
...
- **Prompt to Claude:** "Tell me everything about channel id 42."
- **Expected:** Claude calls `get_channel(channel_id=42)`. Returns formatted block with name, ID, number, group_id, EPG tvg_id, logo presence, stream count with first 10 stream IDs, and auto_created flag.
- **Edge / failure tests:**
  - "Look up channel id 999999 (doesn't exist)" → Backend returns 404; the exception is caught and returned as `"Error getting channel 999999: GET /api/channels/999999 -> HTTP 404 Not Found"`. Confirm no stack trace leaks to the Claude response.
  - "Check the details for channel id 0" → Should trigger a backend 404 or 400; same graceful error handling expected.
- **Watch for (possible bug areas):**
...
- **Prompt to Claude:** "Create a channel called 'Fox News HD' in group 5, number 360."
- **Expected:** Claude calls `create_channel(name='Fox News HD', channel_number=360, group_id=5)`. Tool maps `group_id` → `channel_group_id` in the payload (bd-7q9l3 fix). Response echoes the created channel's actual values: `Channel created: #360: Fox News HD (id=<new_id>, group_id=5)`.
- **Edge / failure tests:**
  ...
  - "Create a channel in group 99999 (non-existent)" → Backend may 400 or silently ignore the group. Verify the returned group_id reflects what actually got set, not what was requested — the tool correctly echoes `result.get("channel_group_id")` from the response.
- **Watch for (possible bug areas):**
...
- **Prompt to Claude:** "Rename channel 42 to 'ESPN Classic' and move it to group 3."
- **Expected:** Claude calls `update_channel(channel_id=42, name='ESPN Classic', group_id=3)`. Tool maps `group_id` → `channel_group_id`. Response echoes the backend's returned state: `Channel 42 updated: name='ESPN Classic', channel_number=<N>, group_id=3`. Critically: the response reflects what the backend returned, not what was sent — so a no-op PATCH (value unchanged) is detectable.
- **Edge / failure tests:**
  - "Update channel 42 but don't change anything" → Claude calls `update_channel(channel_id=42)` with no optional args. Tool hits the `if not payload: return "No changes specified."` guard and returns early without a backend call. Confirm no HTTP request is made.
  - "Update channel 99999 (doesn't exist)" → Backend returns 404; graceful error message expected.
  - "Move channel 42 to group 99999 (non-existent)" → Behavior depends on backend validation. Verify the response's `group_id` field reflects the actual persisted value, not the requested value.
- **Watch for (possible bug areas):**
  - Same `group_id` → `channel_group_id` mapping as `create_channel` — this was the bd-7q9l3 fix class. If this mapping regresses, `update_channel(group_id=5)` silently ignores the group change and returns a 200 with the old group.
...
- **Prompt to Claude:** "Delete channel 42." (Use `get_channel` first to confirm you have the right one.)
- **Expected:** Claude calls `delete_channel(channel_id=42)`. Backend returns 204; tool returns `"Channel 42 deleted."`. Confirm the channel no longer appears in `list_channels`.
- **Edge / failure tests:**
  - "Delete channel 99999 (doesn't exist)" → Backend returns 404; graceful error: `"Error deleting channel 99999: GET ... -> HTTP 404 Not Found"`.
  - "Delete channel 42 a second time (already deleted)" → Same 404 path; no crash.
- **Watch for (possible bug areas):**
  - No confirmation prompt before deletion — the tool fires immediately. There is no "are you sure?" guard; a Claude agent running on an ambiguous prompt could delete the wrong channel with no recourse. This is the expected design, but operators should be coached to always `get_channel` first.
  - The success message is `f"Channel {channel_id} deleted."` — it echoes the input ID, not any backend-confirmed state. If the backend silently succeeded on a different channel (routing bug), the operator would not know.
...
- **Prompt to Claude:** "Delete channels 101, 102, and 103 all at once."
- **Expected:** Claude calls `bulk_delete_channels(channel_ids=[101, 102, 103])`. Tool issues three sequential `channels_delete` calls. Response: `"Bulk delete complete: 3 deleted, 0 errors out of 3 requested."` Verify all three channels are gone.
- **Edge / failure tests:**
  - "Bulk delete [200, 99999, 201] where 99999 doesn't exist" → Should return `"Bulk delete complete: 2 deleted, 1 errors out of 3 requested."` with the first error detail included. Confirm the two valid channels were deleted despite the mid-list error.
...
- **Prompt to Claude:** "Add stream 77 to channel 42."
- **Expected:** Claude calls `add_stream_to_channel(channel_id=42, stream_id=77)`. Returns `"Stream 77 added to channel 42."` Confirm via `get_channel(42)` that stream 77 now appears in the stream list.
- **Edge / failure tests:**
  - "Add stream 99999 (doesn't exist) to channel 42" → Backend should 404 or 400; graceful error string expected.
  - "Add stream 77 to channel 42 again (already present)" → Backend behavior varies — Dispatcharr may silently dedup or return an error. Confirm the tool handles both without crashing.
  - "Add stream 77 to channel 99999 (doesn't exist)" → Backend 404; graceful error.
- **Watch for (possible bug areas):**
...
- **Prompt to Claude:** "Add the stream 'NBC Sports HD' to group 7 and let me know if there's a duplicate."
- **Expected:** Claude calls `add_stream(stream_name='NBC Sports HD', group_id=7, dedup_action='prompt')`. If a candidate is found: response contains `action=pending_merge`, a `merge_id`, candidate channel name/id, and confidence percentage. Claude should then prompt the operator: accept or dismiss? If no candidate: a new channel is created and the stream is attached.
- **Edge / failure tests:**
  - "Add stream 'NBC Sports HD' to group 7, force a new channel regardless of duplicates" → Claude calls `add_stream(stream_name='NBC Sports HD', group_id=7, dedup_action='force_new')`. Dedup enqueue is skipped; channel is created and stream attached. Confirm no pending merge row is created.
  - "Add stream 'XYZ Totally Unique 12345' to group 7" → No candidate found; tool proceeds directly to channel create + stream attach. Verify the new channel appears in `list_channels(group_id=7)`.
  ...
  - "Add stream '' (empty name) to group 7" → Backend's enqueue endpoint rejects blank `stream_name` with 400; the tool's enqueue error handler returns a structured `action=error` string.
- **Watch for (possible bug areas):**
...
- **Prompt to Claude:** "Add streams 10, 11, 12, and 13 to channel 42 all in one go."
- **Expected:** Claude calls `bulk_add_streams_to_channel(channel_id=42, stream_ids=[10, 11, 12, 13])`. Returns `"Added 4 stream(s) to channel 42; channel now has N streams."` Confirm via `get_channel(42)` that all four stream IDs are present.
- **Edge / failure tests:**
  - "Add streams [10, 11] to channel 42, where stream 10 is already on the channel" → Response should show `"Added 1 stream(s) to channel 42 (1 already present); channel now has N streams."` and `"Added: [11]"`. Stream 10 is in `skipped`.
  - "Add an empty list of streams to channel 42" → `stream_ids=[]`. Behavior depends on backend — likely `"Added 0 stream(s) to channel 42."` without error. Confirm no crash.
  - "Add streams to channel 99999 (doesn't exist)" → Backend 404; graceful error: `"Error adding streams to channel 99999: ..."`.
- **Watch for (possible bug areas):**
  ...
  - If the backend's response doesn't include `added`, `skipped`, or `total_streams` keys (e.g., a different response schema), all three default to `[]`/`None` and the summary line reads `"Added 0 stream(s) to channel 42."` — a silent lie if streams were actually added.
...
- **Prompt to Claude:** "Set the EPG ID for channel 42 to 'ESPN.us' and channel 43 to 'CNN.us' in one shot."
- **Expected:** Claude calls `bulk_assign_epg(mappings=[{"channel_id": 42, "tvg_id": "ESPN.us"}, {"channel_id": 43, "tvg_id": "CNN.us"}])`. Returns `"Updated EPG assignments for 2/2 channels."` Confirm both channels now have the correct `tvg_id` via `get_channel`.
- **Edge / failure tests:**
  - "Clear the EPG ID for channel 42 by passing an empty tvg_id" → `mappings=[{"channel_id": 42, "tvg_id": ""}]`. The tool sends `{"tvg_id": ""}` to the PATCH endpoint. Confirm the channel's EPG link is cleared, not silently ignored.
  ...
  - "Bulk assign EPG to channel 99999 (doesn't exist)" → PATCH returns 404; error is appended (`"channel 99999: ..."`) and the final error count is surfaced.
- **Watch for (possible bug areas):**
...
- **Prompt to Claude:** "Remove stream 77 from channel 42."
- **Expected:** Claude calls `remove_stream_from_channel(channel_id=42, stream_id=77)`. Returns `"Stream 77 removed from channel 42."` Confirm via `get_channel(42)` that stream 77 is no longer in the stream list.
- **Edge / failure tests:**
  - "Remove stream 77 from channel 42 when it isn't on the channel" → Backend behavior determines the response — may 400 or silently succeed. Confirm graceful error handling either way.
  - "Remove stream 77 from channel 99999 (doesn't exist)" → Backend 404; graceful error string.
- **Watch for (possible bug areas):**
...
- **Prompt to Claude:** "Reorder the streams on channel 42 so stream 77 comes first, then 88, then 99." (First use `get_channel(42)` to get the current stream list.)
- **Expected:** Claude calls `reorder_streams(channel_id=42, stream_ids=[77, 88, 99])`. Returns `"Streams reordered for channel 42. New order: [77, 88, 99]"`. Verify via `get_channel(42)` that the stream order is updated.
- **Edge / failure tests:**
  - "Reorder streams on channel 42 with an incomplete list (omitting stream 99)" → Behavior depends on backend — it may reject the partial list or silently remove stream 99 from the channel. This is a high-risk case: an incomplete `stream_ids` list could detach streams from the channel permanently.
  - "Reorder streams with stream IDs that don't belong to channel 42" → Backend should reject; confirm graceful error handling.
  - "Reorder with an empty list []" → Backend may clear all streams from the channel. This is a destructive edge case with no confirmation guard in the tool.
- **Watch for (possible bug areas):**
...
- **Prompt to Claude:** "Assign sequential channel numbers to channels 101, 102, and 103 starting from 200."
- **Expected:** Claude calls `assign_channel_numbers(channel_ids=[101, 102, 103], starting_number=200)`. Returns `"Assigned numbers to 3 channels starting from 200."` Verify via `get_channel` that channels 101, 102, 103 now have numbers 200, 201, 202 respectively.
- **Edge / failure tests:**
  - "Auto-assign channel numbers to channels 101, 102, 103 (no starting number)" → Claude calls without `starting_number`. Response: `"Assigned numbers to 3 channels starting from auto."`. Confirm the backend chooses sensible numbers.
...
- **Prompt to Claude:** "Merge channels 102 and 103 into channel 101 — keep channel 101 and delete the others." (Use `list_channels` first to confirm IDs.)
- **Expected:** Claude calls `merge_channels(target_channel_id=101, source_channel_ids=[102, 103])`. Tool calls `channels_bulk_merge` with one merge item. Returns `"Merged 2 channels into channel 101."`. Confirm channels 102 and 103 are deleted and channel 101 now contains their streams.
- **Edge / failure tests:**
  - "Merge channel 102 into channel 99999 (target doesn't exist)" → Backend should 404 or 422 on the bulk-merge request. Graceful error: `"Error merging channels: POST /api/channels/bulk-merge -> HTTP 404 ..."`.
  - "Merge channel 101 into itself (source_channel_ids=[101])" → Backend behavior is undefined; may 400 or silently no-op. Confirm no crash.
  - "Merge with an empty source list []" → `source_channel_ids=[]`. `merged = len([])` = 0 so response is `"Merged 0 channels into channel 101."`. Whether the backend accepts an empty sources list is untested.
- **Watch for (possible bug areas):**
  - The return message `f"Merged {merged} channels into channel {target_channel_id}."` counts `len(source_channel_ids)` and is built from inputs. The actual backend response from `call_endpoint` is completely discarded (no `result` variable). If the backend partially failed (some sources merged, some errored), the tool still reports full success with the requested count.
...
- **Prompt to Claude:** "Clear all auto-created channels from groups 5 and 6."
- **Expected:** Claude calls `clear_auto_created(group_ids=[5, 6])`. Returns `"Cleared N auto-created channels in 2 groups."` Confirm channels with `auto_created=True` in those groups are gone.
- **Edge / failure tests:**
  ...
  - "Clear auto-created channels from group 99999 (no auto-created channels there)" → Backend should return 0 deleted. Response: `"Cleared 0 auto-created channels in 1 groups."` — grammatically odd but functionally correct.
...
- **Prompt to Claude:** "Merge the duplicates I found: keep channel 101 and absorb 102 and 103; also keep channel 200 and absorb 201." (Use `find_duplicate_channels` first to get the IDs.)
- **Expected:** Claude calls `bulk_merge_duplicate_channels(merges=[{"target_channel_id": 101, "source_channel_ids": [102, 103]}, {"target_channel_id": 200, "source_channel_ids": [201]}])`. Returns a summary: `"Bulk merge complete: 2 merged, 0 failed."` followed by per-merge results showing target name, sources absorbed, and total streams.
- **Edge / failure tests:**
  - "Bulk merge where one target doesn't exist" → Backend returns a result entry with `success=False` for that merge. The tool shows `"✗ Channel <id>: <error>"`. The count is `1 merged, 1 failed`.
...
- **Prompt to Claude:** "Create two new channels atomically: 'Test Channel A' at number 900 and 'Test Channel B' at number 901, both in group 4. Use validate-only mode first."
- **Expected:** Claude calls `bulk_commit_channels(operations=[{"type": "createChannel", "tempId": -1, "name": "Test Channel A", "channelNumber": 900, "groupId": 4}, {"type": "createChannel", "tempId": -2, "name": "Test Channel B", "channelNumber": 901, "groupId": 4}], validate_only=True)`. Response: `"Bulk commit SUCCESS: 2 operations submitted. (validate-only mode — no changes applied)"`. Then repeat without `validate_only=True` to actually apply.
- **Edge / failure tests:**
...
- **Prompt to Claude:** "Set the logos for channels 42, 43, and 44 using their EPG entries."
- **Expected:** Claude calls `set_logo_from_epg(channel_ids=[42, 43, 44])`. For each channel: reads the channel, looks up EPG data by `epg_data_id`, POSTs to `/api/channels/logos` to create/find the logo, then PATCHes the channel. Returns summary: `"Set logos from EPG: 3 assigned, 0 skipped (no EPG link), 0 skipped (no icon_url), 0 errors out of 3 requested."`.
- **Edge / failure tests:**
  - "Set logos for a channel with no EPG link (epg_data_id is null)" → That channel is counted in `skipped_no_epg`. Summary shows 0 assigned, 1 skipped (no EPG link).
  ...
  - "Set logos for two channels that share the same EPG icon_url" → Logo cache (`logo_cache`) should reuse the same `logo_id` for the second channel, avoiding a duplicate logo POST. Verify only one logo creation call is made.
  - "Set logos for channel 99999 (doesn't exist)" → `client.get(f"/api/channels/{cid}")` returns 404; exception caught and added to `errors`. Summary: `"0 assigned, ... 1 errors"`.
- **Watch for (possible bug areas):**
...
- **Prompt to Claude:** "Build a lineup in group 8 with these channels: Fox News at 360, CNN at 200, MSNBC at 356. Match streams from provider 3 in the east market."
- **Expected:** Claude calls `build_channel_lineup(channels=[{"name": "Fox News", "number": 360}, {"name": "CNN", "number": 200}, {"name": "MSNBC", "number": 356}], group_id=8, provider_id=3, market="east")`. Tool bulk-creates all three, fetches the group, fuzzy-matches each to a stream, and assigns them. Summary: `"Lineup built: 3 channels created, 3 streams matched, 0 unmatched."` Confirm via `list_channels(group_id=8)` that all three channels exist with streams assigned.
- **Edge / failure tests:**
...
- **Edge / failure tests:**
  - "List merge history — show me dismissed merges from group 5" → Claude calls `list_pending_channel_merges(group_id=5, status='dismissed')`. Confirm `status='dismissed'` is sent and results are filtered to group 5.
...
- **Prompt to Claude:** "Accept the pending merge with id 15 — confirm it should be merged into that existing channel." (Use `list_pending_channel_merges` first to get a valid `merge_id`.)
- **Expected:** Claude calls `accept_channel_merge(merge_id=15)`. Returns a dict: `{merged_into_channel_id, journal_entry_id, source_stream_id, confidence, status: 'merged'}`. Confirm via `list_pending_channel_merges(status='merged')` that the row now appears as merged.
- **Edge / failure tests:**
  - "Accept the same merge id 15 again (idempotency)" → Tool returns the same outcome envelope as the first call. Response includes `status: 'merged'` and the original `journal_entry_id`. No error.
  - "Accept a merge id where the target channel was deleted from Dispatcharr" → Backend returns 404; tool catches it and returns `{"error": {"code": "TARGET_NOT_FOUND", "message": "..."}}`. Claude should then prompt the operator to call `dismiss_channel_merge(15)` to clean up the stale row.
  - "Accept a merge id that has already been dismissed" → Backend returns 409; tool catches it and returns `{"error": {"code": "INVALID_STATE", "message": "..."}}`. Claude should NOT retry.
  - "Accept a merge id that doesn't exist (99999)" → Backend returns 404 for the pending row (not the target channel). `_http_status_code` returns 404; tool returns `TARGET_NOT_FOUND` error envelope. Confirm the error message makes sense in this context — the message says "target channel no longer exists" but the actual issue is the merge row itself doesn't exist.
  - "Accept merge id 15 where the stream name matches multiple Dispatcharr streams" → Backend logs a WARN and records the operator decision in the audit trail without adding the stream to the channel. Return envelope still shows `status: 'merged'`. Operator must manually assign the stream.
- **Watch for (possible bug areas):**
  - The 404 error envelope message says "Pending merge id=N: target channel no longer exists in Dispatcharr" — but a 404 from `accept_channel_merge` could also mean the `pending_merges` row itself doesn't exist (the row 404 from the backend). The error message conflates two different 404 causes; an operator acting on merge_id=99999 (never existed) gets advice to call `dismiss_channel_merge` which will itself 404. This could confuse the operator.
...
- **Prompt to Claude:** "Dismiss the pending merge id 15 — that suggested merge is wrong, don't merge those channels." (Use `list_pending_channel_merges` first to get a valid `merge_id`.)
- **Expected:** Claude calls `dismiss_channel_merge(merge_id=15)`. Returns `{journal_entry_id, status: 'dismissed'}`. Confirm via `list_pending_channel_merges(status='dismissed')` that the row is now dismissed.
- **Edge / failure tests:**
  - "Dismiss the same merge id 15 again (idempotency)" → Returns the original outcome envelope with the same `journal_entry_id`. No error.
  - "Dismiss a merge that has already been accepted (cross-state)" → Backend returns 409; tool catches it and returns `{"error": {"code": "INVALID_STATE", "message": "Pending merge id=15 is already in a terminal state that does not allow dismissal (e.g. already merged). Do not retry this operation."}}`. Confirm Claude does NOT retry.
  - "Dismiss a merge id that doesn't exist (99999)" → Backend returns 404 for the pending row. `_http_status_code` returns 404 — but the `dismiss_channel_merge` tool only catches 409! A 404 falls through to the re-raise path. Claude sees a raw exception, not a structured error envelope. This is a gap: the tool handles 409 gracefully but lets 404 propagate.
  - "Use dismiss as recovery after accept returns TARGET_NOT_FOUND" → Claude calls `dismiss_channel_merge(merge_id=15)` after receiving the TARGET_NOT_FOUND envelope. Should succeed with `status: 'dismissed'`, cleaning up the stale row.
- **Watch for (possible bug areas):**
  - Critical gap: the tool handles 409 (INVALID_STATE) gracefully with a structured error envelope, but does NOT handle 404 (row not found). A 404 from a non-existent `merge_id` re-raises as an unhandled exception. This is inconsistent with `accept_channel_merge` which does handle 404 (even if the message is ambiguous). An operator typing the wrong merge_id gets a raw exception from `dismiss` but a structured error from `accept`.
...
- **Prompt to Claude:** "Check the health of stream 1042." (Use a real ID from `list_streams` first.)
- **Expected:** Claude calls `probe_single_stream(stream_id=1042)`. Response: "Stream 1042 probe complete. Status: \<status\>". The status comes from `result.get("status", result.get("probe_status", "unknown"))` — expect values like "success", "failed", or "timeout".
- **Edge / failure tests:**
  - "Probe stream ID 999999 (non-existent)" → backend should 404; `_http_error` converts it to a RuntimeError; Claude returns "Error probing stream 999999: POST /api/stream-stats/probe/999999 -> HTTP 404 Not Found".
  - "Probe stream ID -1 or 0" → no client-side guard; backend validation is the only gate.
...
- **Prompt to Claude:** "Remove streams 201, 202, and 203 from channel 45." (Use IDs from `get_streams_for_channel` first.)
- **Expected:** Claude calls `bulk_remove_streams(channel_id=45, stream_ids=[201, 202, 203])`. The tool fetches the current stream list via `GET /api/channels/45`, filters out the specified IDs, and PATCHes back the remaining list. Response: "Removed N streams from channel 45. Remaining: M streams."
- **Edge / failure tests:**
  - "Remove streams that are not actually in channel 45" → `actually_removed == 0`; Claude returns "None of the specified streams were in channel 45." — clean no-op.
  - "Remove all streams from a channel (empty the stream list)" → `filtered = []`; the PATCH sends `{"streams": []}` — verify the backend accepts an empty list and doesn't 400. This is a potentially dangerous operation that leaves a channelless channel silently.
...
- **Prompt to Claude:** "List all streams assigned to channel 12." (Use a real ID from `list_streams` or the channel list.)
- **Expected:** Claude calls `get_streams_for_channel(channel_id=12)`. Response lists numbered streams with name, ID, group, and provider. Hits `GET /api/channels/12/streams`.
- **Edge / failure tests:**
  - "Get streams for a channel with no streams assigned" → `if not streams:` returns "Channel 12 has no streams assigned." — clean.
  - "Get streams for channel ID 0 or a negative ID" → no client-side guard; backend 404 is surfaced via `_http_error`.
...
- **Prompt to Claude:** "Give me details on streams 101, 205, and 310." (Use IDs retrieved from a prior search.)
- **Expected:** Claude calls `get_streams_by_ids(stream_ids=[101, 205, 310])`. Response: "Found N of 3 requested streams:" with name, ID, group, and provider per stream.
- **Edge / failure tests:**
  - "Request IDs that include one non-existent ID, like [101, 9999999]" → backend returns only the found streams; Claude reports "Found 1 of 2 requested streams" — the operator can see which are missing by count delta.
  ...
  - "Pass a list with duplicate IDs: [101, 101, 101]" → backend deduplicates or not; the "Found N of 3" count may look odd.
...
- **Prompt to Claude:** "Probe these three streams for health: 101, 205, 310." (Use IDs from a prior search.)
- **Expected:** Claude calls `probe_bulk_streams(stream_ids=[101, 205, 310])`. Response: "Bulk probe completed for 3 streams: Success: N, Failed: M" plus a list of failed stream names/errors (up to 20 shown).
- **Edge / failure tests:**
  - "Probe bulk streams with a list containing a non-existent ID" → backend should probe what it can; the non-existent ID should appear in "failed" with an appropriate error.
...
- **Prompt to Claude:** "Auto-match streams to all unassigned channels in group 3." (Use a real group ID from `list_channel_groups`.)
- **Expected:** Claude calls `match_streams_to_channels(group_id=3)`. The tool paginates through all channels in the group (`page_size=500`), filters to those with 0 streams, fuzzy-matches each channel name, and POSTs `add-stream` for each hit. Response: "Matched N of M unassigned channels in group 3:" followed by `#NUM ChannelName → StreamName (id=ID)` for each match, then an "Unmatched" section for failures.
- **Edge / failure tests:**
  - "Run on a group where all channels already have streams" → `if not unassigned:` returns "All N channels in group 3 already have streams assigned." — clean.
  - "Run on a non-existent group ID" → `channels_list` returns empty; `if not all_channels:` returns "No channels found in group 999."
...
- **Prompt to Claude:** "Apply channel profile 2 to channels 10, 11, and 12." (Use a real profile ID from `list_channel_profiles` first.)
- **Expected:** Claude calls `apply_profile_to_channels(profile_id=2, channel_ids=[10, 11, 12])`. The tool PATCHes the profile, then reads back the profile list to confirm the new channel count. Response: "Profile 2 applied to 3 channels. Profile now has N channels."
- **Edge / failure tests:**
  - "Apply a profile ID that doesn't exist" → PATCH returns 404; `_http_error` surfaces it; Claude returns "Error applying profile: PATCH ... -> HTTP 404 Not Found".
  - "Apply to an empty channel_ids list `[]`" → PATCH body `{"channel_ids": []}` — may be a bulk-clear operation on the backend. No guard in the tool; operator should be warned.
  - "Apply profile to a channel_ids list with a non-existent channel ID" → backend may silently skip the bad ID or 400; verify which behavior occurs.
...
- **Prompt to Claude:** "Refresh M3U account number 3." (use an id from `list_m3u_accounts` first)
- **Expected:** Claude calls `refresh_m3u(account_id=3)`. Response is `"M3U account 3 refresh started. <backend message>"`. The backend triggers Dispatcharr refresh immediately and spawns a background poll task; the MCP tool returns before completion. Claude should communicate that the refresh is asynchronous.
- **Edge / failure tests:**
  - "Refresh M3U account 9999" (non-existent id) → backend returns 500, MCP tool should return `Error refreshing M3U account 9999: GET /api/m3u/accounts/9999 -> HTTP 500 ...` not an unhandled exception.
  - "Refresh M3U account 2" when a refresh is already running for that account → no lock prevents double-trigger; both will run. Watch for the result message — it will just say "refresh started" regardless. Note this for the operator.
- **Watch for (possible bug areas):** `result.get("message", "")` fails silently if the backend returns a non-dict (e.g., a list or `None`). The `isinstance(result, dict)` guard is present but the fallback is an empty string, so `msg` will be blank and Claude will see a bare "M3U account 3 refresh started." with no server detail. The 300s timeout is set on the `call_endpoint` call, but the backend's refresh endpoint itself returns immediately and spawns a background poll — the timeout only guards the trigger call, not actual completion. The operator should be aware: "started" does not mean "finished."
...
- **Prompt to Claude:** "Show me the full details for M3U account 2." (use an id from `list_m3u_accounts` first)
- **Expected:** Claude calls `get_m3u_account(account_id=2)`. Response includes name, id, type, URL (truncated at 60 chars), status, stream count, and last refresh timestamp. URL truncation is deliberate — `'...'` appended only if `len(url) > 60`.
- **Edge / failure tests:**
  - "Get details for M3U account 9999" (non-existent) → backend returns HTTP 500 (the backend router swallows the Dispatcharr 404 into a 500); MCP tool returns `Error getting M3U account 9999: GET /api/m3u/accounts/9999 -> HTTP 500`. No 404-specific message surfaced.
  - "Get M3U account 0" (invalid id) → likely 500 from backend; same graceful error path.
...
- **Prompt to Claude:** "Add a new M3U account called 'Test Provider' with the URL https://example-iptv.com/playlist.m3u8 — WARNING: this will actually create an account in ECM."
- **Expected:** Claude calls `create_m3u_account(name="Test Provider", url="https://example-iptv.com/playlist.m3u8", server_type="standard")`. Response is `"M3U account created: Test Provider (id=<N>)"`. Backend posts `{"name": ..., "url": ..., "server_type": ...}` to `POST /api/m3u/accounts`.
- **Edge / failure tests:**
  - "Create an M3U account with url=http://192.168.1.100/list.m3u" (private IP / non-https URL) → `validate_url_scheme` in the backend may reject it; tool should surface the HTTP 500 detail. Test whether the URL scheme validator allows `http://` or only `https://`.
  - "Create an M3U account named 'Test Provider' again" (duplicate name) → Dispatcharr may return 400 conflict; backend swallows to 500; tool returns `Error creating M3U account: POST /api/m3u/accounts -> HTTP 500`. No deduplication logic (unlike `create_channel_group` which has a 400-catch-and-return path).
...
- **Prompt to Claude:** "Rename M3U account 3 to 'Primary Provider'." (use an id from `list_m3u_accounts` first)
- **Expected:** Claude calls `update_m3u_account(account_id=3, name="Primary Provider")`. Response is `"M3U account 3 updated: name='Primary Provider', url='<current url>'"`. Payload only includes fields that were set — `name` only here. The backend hits `PATCH /api/m3u/accounts/{id}`.
- **Edge / failure tests:**
  - "Update M3U account 3 but don't change anything" (no name, no url provided) → tool catches this client-side and returns `"No changes specified."` without making an HTTP call.
  - "Change the URL for M3U account 99 to https://newprovider.com/list.m3u" (non-existent id) → backend 500; tool returns error string.
...
- **Prompt to Claude:** "Delete M3U account 5." (use an id from `list_m3u_accounts` first; confirm this is intentional)
- **Expected:** Claude calls `delete_m3u_account(account_id=5)`. Response is `"M3U account 5 deleted."`. Backend actually does more: it fetches the account's channel groups, identifies non-shared ones, deletes the account, then deletes the orphaned groups, cleans up `linked_m3u_accounts` in settings, and journals the deletion. The MCP tool returns no information about the cascade — how many groups were deleted is not surfaced.
- **Edge / failure tests:**
  - "Delete M3U account 9999" (non-existent) → backend 500; tool returns `Error deleting M3U account 9999: DELETE /api/m3u/accounts/9999 -> HTTP 500`.
  - "Delete M3U account 3" (account shared with another via server groups) → backend identifies shared groups and skips deleting them. MCP tool still returns `"M3U account 3 deleted."` — the operator gets no indication that shared groups were retained.
...
- **Prompt to Claude:** "Disable the 'Sports HD' group on M3U account 2." (use an account id from `list_m3u_accounts` and a group name that exists on that account)
- **Expected:** Claude calls `update_m3u_group_settings(account_id=2, group_name="Sports HD", enabled=False)`. Response is `"Group 'Sports HD' disabled on M3U account 2."`. The tool sends `PATCH /api/m3u/accounts/2/group-settings` with body `{"Sports HD": False}`.
- **Edge / failure tests:**
  - "Enable the 'Movies 4K' group on account 2" where that group name does not exist on account 2 → the backend receives `{"Movies 4K": True}` and will likely silently ignore the unknown key or return a 400. Watch whether the tool surfaces the error or returns the success message regardless (the tool's response is built from the state change declaration, not from the HTTP response body).
  - "Disable 'Sports HD' group on account 99" (non-existent account) → backend 500; tool returns error.
...
- **Prompt to Claude:** "On M3U account 2, disable the 'Sports HD' and 'News' groups, and enable the 'Movies' group all at once."
- **Expected:** Claude calls `bulk_update_m3u_group_settings(account_id=2, groups={"Sports HD": False, "News": False, "Movies": True})`. Response lists each change: `"Updated 3 groups on M3U account 2:\n  disabled 'Sports HD'\n  disabled 'News'\n  enabled 'Movies'"`. Same PATCH endpoint as `update_m3u_group_settings` but with multiple keys.
- **Edge / failure tests:**
  - "Update groups with an empty dict" (`groups={}`) → the tool sends an empty-body PATCH. Backend receives `{}` — behavior is Dispatcharr-defined. The tool's `changes` list will be empty and the response will be `"Updated 0 groups on M3U account 2:\n  "` (the join produces a trailing newline with no entries — minor formatting issue).
  - "Bulk update with 50 groups" on account 2 → all 50 group-name keys sent in one PATCH body. If any names don't exist, Dispatcharr may silently ignore them or reject the entire request. The tool does not report which keys were rejected.
...
- **Prompt to Claude:** "Refresh EPG source 1." (use an id from `list_epg_sources` first)
- **Expected:** Claude calls `refresh_epg(source_id=1)`. Response is `"EPG source 1 refresh started. <backend message>"`. Backend triggers Dispatcharr refresh and spawns a background poll task (up to 15 minutes wait vs. M3U's 5 minutes). Tool returns before completion.
- **Edge / failure tests:**
  - "Refresh EPG source 9999" (non-existent) → backend 500; tool returns `"Error refreshing EPG source 9999: ..."`.
  - "Refresh EPG source 1" twice rapidly → two concurrent background pollers for the same source. Second trigger may succeed or fail depending on Dispatcharr's concurrency handling. No guard in MCP or backend.
...
- **Prompt to Claude:** "Refresh all my EPG sources." (or "Refresh EPG sources 2 and 4 only.")
- **Expected:** Claude calls `refresh_all_epg()` or `refresh_all_epg(source_ids=[2, 4])`. Response is `"Refreshed N/M EPG sources."` plus an errors block if any failed. Unlike `refresh_all_m3u`, this loops and calls `refresh_epg_source` per source individually, collecting partial failures.
- **Edge / failure tests:**
  - "Refresh EPG sources [1, 9999]" where 9999 doesn't exist → source 1 refreshes, source 9999 fails; tool reports `Refreshed 1/2 EPG sources.\nErrors (1):\n  - source 9999: GET /api/epg/sources/9999 -> ...`. Verify the error is surfaced in the output.
...
- **Prompt to Claude:** "Add a new EPG source called 'US Guide' with URL https://xmltv.example.com/us-guide.xml.gz — WARNING: this will actually create an EPG source."
- **Expected:** Claude calls `create_epg_source(name="US Guide", url="https://xmltv.example.com/us-guide.xml.gz")`. Response is `"EPG source created: US Guide (id=<N>)"`. Backend validates the URL scheme and forwards to Dispatcharr.
- **Edge / failure tests:**
  - "Create EPG source with URL file:///etc/passwd" → `validate_url_scheme` should reject non-http/https; backend returns 400/500; tool surfaces the error.
  - "Create EPG source 'US Guide' again" (duplicate name) → Dispatcharr may 400; backend swallows to 500; tool returns error. No dedup path like channel groups.
...
- **Prompt to Claude:** "Change the URL for EPG source 3 to https://newguide.example.com/guide.xml." (use an id from `list_epg_sources` first)
- **Expected:** Claude calls `update_epg_source(source_id=3, url="https://newguide.example.com/guide.xml")`. Response is `"EPG source 3 updated: name='<current name>', url='https://newguide.example.com/guide.xml'"`. No call is made if neither `name` nor `url` is provided.
- **Edge / failure tests:**
  - "Update EPG source 3 with no changes" → tool returns `"No changes specified."` without making an HTTP call.
  - "Rename EPG source 9999 to 'New Name'" (non-existent) → backend 500; tool returns error with HTTP detail.
...
- **Prompt to Claude:** "Delete EPG source 4." (use an id from `list_epg_sources` first; confirm this is a disposable source)
- **Expected:** Claude calls `delete_epg_source(source_id=4)`. Response is `"EPG source 4 deleted."`. Backend fetches the source name before deletion for journal logging, then deletes via Dispatcharr.
- **Edge / failure tests:**
  - "Delete EPG source 9999" (non-existent) → backend 500; tool returns `"Error deleting EPG source 9999: ..."`.
  - "Delete EPG source 4" twice → second call returns backend 500 (source already gone); tool returns error.
...
- **Prompt to Claude:** "What's on TV right now according to the EPG?" (or "Show me the EPG schedule for channel 5, up to 10 programs.")
- **Expected:** Claude calls `get_epg_grid()` or `get_epg_grid(channel_id=5, limit=10)`. Response is a formatted schedule list: `EPG Schedule (N programs): [ChannelName] Title (start - end)`. The backend returns the full grid; `channel_id` and `limit` filtering happen client-side in the MCP tool.
- **Edge / failure tests:**
  - "Show EPG grid for channel 9999" (channel with no programs or non-existent) → backend returns full grid, tool filters for `channel_id=9999`, finds nothing, returns `"No EPG schedule data available."` This is correct but could confuse if 9999 is a valid channel with no EPG data vs. a non-existent channel.
...
- **Watch for (possible bug areas):** `channel_id` filtering uses `channel_id in (p.get("channel_id"), p.get("channel"))` — the program dict key might be `"channel"` (an ID) or `"channel_id"`. If neither matches (e.g., the key is `"channel_name"` for a string), ALL programs are filtered out and the result is `"No EPG schedule data available."` even with data present. The `limit` param is applied after channel filtering with `programs[:limit]` — so a `limit=1` with `channel_id` set will return at most 1 program from that channel, which is correct. The `limit` default is 20 and is not capped server-side — the operator could ask for `limit=10000` and get a very long response.
...
- **Prompt to Claude:** "Delete channel group 7." (use an id from `list_channel_groups`; for a test, use a group you know is empty)
- **Expected:** Claude calls `delete_channel_group(group_id=7)`. Response is `"Channel group 7 deleted."` OR (if the group has M3U sync settings) the backend hides the group and returns `{"status": "hidden"}` — but the MCP tool ignores the response and always reports `"Channel group 7 deleted."` even if it was actually hidden not deleted. With `delete_channels=True`: MCP tool paginates through channels in the group (page_size=500), deletes each individually, then deletes the group, then does a read-back verification.
- **Edge / failure tests:**
  - "Delete channel group 7" where it has active M3U sync → backend hides it (returns `{"status": "hidden"}`); MCP tool reports `"Channel group 7 deleted."` — misleading to the operator. Run `get_hidden_groups()` afterward to verify actual state.
  - "Delete channel group 7 with delete_channels=True" where some channels fail to delete → best-effort: partial deletion continues, failed channels are logged but not reported to MCP. The group delete is then attempted; if channels still exist, the group delete may fail on the backend. Tool will show `"Error deleting channel group 7: ..."`.
  - "Delete channel group 9999" (non-existent) → backend 500; tool returns error.
- **Watch for (possible bug areas):** The MCP tool always reports `"Channel group 7 deleted."` regardless of whether the backend actually deleted or hid the group — the response dict `{"status": "hidden", "message": "..."}` is never checked. This is a confirmed behavioral gap. The read-back verification after deletion calls `list_channel_groups` (which excludes hidden groups) — if the group was hidden rather than deleted, it will NOT appear in the list, so `still_present` will be `False` and the tool will not issue the WARNING. The hidden state is effectively indistinguishable from deleted from the tool's perspective. With `delete_channels=True`, individual channel deletes have no concurrency — they are strictly serial, which is slow for large groups.
...
- **Edge / failure tests:**
  ...
  - "Show hidden groups" after a `delete_channel_group` call that silently hid a group → the group should appear here. Use this as a verification step after any `delete_channel_group` call.
...
- **Purpose:** Delete orphaned channel groups (those with no streams, channels, or M3U association). Optionally targets specific IDs; if None, deletes all orphaned.
- **Prompt to Claude:** "Clean up all orphaned channel groups." (or "Delete orphaned groups 12 and 15 only.")
- **Expected:** Claude calls `delete_orphaned_groups()` or `delete_orphaned_groups(group_ids=[12, 15])`. Response lists deleted groups: `"Deleted N orphaned group(s):\n  <name> (id=<N>)"`. If `result` is `None`, returns `"No orphaned groups were deleted."`.
- **Edge / failure tests:**
  - "Delete orphaned groups [12, 999]" where 12 is orphaned and 999 is not → backend filters to actually-orphaned groups, then further filters by the specified IDs. Group 999 is not orphaned so it is not considered; only group 12 is deleted. Result: `"Deleted 1 orphaned group(s): Group12Name (id=12)"`. Verify 999 is not touched.
...
- **Prompt to Claude:** "Show me the details of auto-creation rule 3." (First use `list_auto_creation_rules` to get a real id.)
- **Expected:** Claude calls `get_auto_creation_rule(rule_id=3)`. Returns rule name, id, enabled state, priority, description (if set), run_on_refresh, stop_on_first_match, skip_struck_streams, orphan_action, sort settings, normalization group ids (if set), up to 10 conditions with type and value, and up to 10 actions with type and target/value. Conditions/actions beyond 10 show "... and N more".
- **Edge / failure tests:**
  - "Get auto-creation rule 99999" (non-existent) → backend returns 404; tool should return `Error getting rule 99999: GET /api/auto-creation/rules/99999 -> HTTP 404 Not Found: ...`
...
- **Prompt to Claude:** "Disable auto-creation rule 2." (Get the id from `list_auto_creation_rules` first.) Then: "Now re-enable it."
- **Expected:** Claude calls `toggle_auto_creation_rule(rule_id=2)`. Returns "Rule 2 is now disabled." On second call returns "Rule 2 is now enabled." The toggle is idempotent from the UI's perspective — calling it twice returns you to the original state.
- **Edge / failure tests:**
  - "Toggle rule 99999" (non-existent id) → backend 404; tool returns `Error toggling rule 99999: ...`
...
- **Prompt to Claude:** "Toggle rules 1, 2, and 3 all at once." (Use real ids from `list_auto_creation_rules`.)
- **Expected:** Claude calls `bulk_toggle_auto_creation_rules(rule_ids=[1, 2, 3])`. Returns "Toggled 3/3 rules:" with one "Rule N: enabled/disabled" line per id. If any id fails, the Errors section lists each failure without aborting the rest.
- **Edge / failure tests:**
  - Mix of valid and invalid ids (e.g., `[1, 99999, 2]`) → should toggle 1 and 2 successfully, report error for 99999; final output is "Toggled 2/3 rules:" with the Errors section visible.
  - Empty list `rule_ids=[]` → returns "Toggled 0/0 rules:" (no crash on empty loop).
  - Single-element list → equivalent to `toggle_auto_creation_rule`; both paths should give the same result.
...
- **Prompt to Claude:** "Duplicate auto-creation rule 1 so I can modify the copy." (Use a real id from `list_auto_creation_rules`.)
- **Expected:** Claude calls `duplicate_auto_creation_rule(rule_id=1)`. Returns "Rule 1 duplicated. New rule ID: N". Run `list_auto_creation_rules` afterward to verify the new rule appears.
- **Edge / failure tests:**
  - "Duplicate rule 99999" → backend 404; tool returns `Error duplicating rule 99999: ...`
...
- **Prompt to Claude:** "Delete auto-creation rule 5." (Use a real id from `list_auto_creation_rules`. Duplicate it first if you want to preserve the original.)
- **Expected:** Claude calls `delete_auto_creation_rule(rule_id=5)`. Returns "Rule 5 deleted." Rule no longer appears in `list_auto_creation_rules`. This is irreversible.
- **Edge / failure tests:**
  - "Delete rule 99999" → backend 404; tool returns `Error deleting rule 99999: ...`
  ...
  - Delete a rule, then call `get_auto_creation_rule` with the same id → must return a clean error, not crash.
...
- **Prompt to Claude:** "Update auto-creation rule 3: change its priority to 5 and enable it." (Use a real id from `list_auto_creation_rules`.)
- **Expected:** Claude calls `update_auto_creation_rule(rule_id=3, priority=5, enabled=True)`. Payload is `{"priority": 5, "enabled": True}` (only supplied fields). Returns "Updated rule 'Name' (id=3). Changed: priority, enabled".
- **Edge / failure tests:**
  - "Update rule 3 with no changes" (pass no optional fields) → tool returns "No fields to update." without making any backend call. Verify this early-exit path.
  - "Update rule 99999" (non-existent id) → backend 404; tool returns `Error updating rule 99999: ...`
...
- **Prompt to Claude:** "Roll back auto-creation execution 7." (Get a real non-dry-run execution id from `list_auto_creation_executions` first.)
...
- **Edge / failure tests:**
  - Roll back execution 99999 (non-existent) → backend returns 400 with "Execution not found"; tool returns `Error rolling back execution 99999: POST /api/auto-creation/executions/99999/rollback -> HTTP 400 Not Found: Execution not found`
  ...
  - Roll back an already-rolled-back execution → backend returns 400 "Execution already rolled back"; tool must surface this, not appear to succeed.
  - Roll back a currently-running execution → behavior is undefined; test and document.
...
- **Prompt to Claude:** "Run the M3U refresh task right now." (Get the task_id from `list_tasks` first — e.g., `m3u_refresh`.)
...
- **Edge / failure tests:**
  - "Run task 'nonexistent_task'" → backend 404; tool returns `Error running task 'nonexistent_task': POST /api/tasks/nonexistent_task/run -> HTTP 404 ...`
...
- **Prompt to Claude:** "Cancel the stream probe task." (Start it first with `run_task` to have something to cancel. Get task_id from `list_tasks`.)
...
- **Edge / failure tests:**
  - Cancel a task that is not running → backend returns `{"status": "not_running", "message": "Task stream_probe is not running"}` as HTTP 200 (not 404). The MCP tool reads `result.get("message", "")` and returns "Task 'stream_probe' cancelled. Task stream_probe is not running" — which is misleading (says "cancelled" but wasn't). Verify this messaging.
  - Cancel a task with non-existent task_id → backend raises 404; tool returns `Error cancelling task 'bad_id': POST /api/tasks/bad_id/cancel -> HTTP 404 Not Found: ...`
...
- **Prompt to Claude (specific):** "Show me the last 5 runs of the M3U refresh task." (Use a real task_id from `list_tasks`.)
...
- **Edge / failure tests:**
  - Task with no history → "No task history for task 'm3u_refresh'."
...
- **Prompt to Claude:** "Show me the schedules for the M3U refresh task." (Use a real task_id from `list_tasks`.)
...
- **Edge / failure tests:**
  - Unknown task_id → backend may return 404 or empty list; verify tool handles both without crashing.
...
- **Prompt to Claude (weekly):** "Schedule the channel cleanup task every Monday and Wednesday at 2:00 AM."
...
- **Prompt to Claude:** "Delete schedule 3 from the M3U refresh task." (Get real ids from `list_task_schedules` first.)
...
- **Edge / failure tests:**
  ...
  - Non-existent task_id → backend 404; same error surfacing.
...
- **Purpose:** Get channel viewing statistics including active viewers and stream status.
- **Prompt to Claude:** "How many viewers are watching channels right now? Show me which channels are active."
- **Expected:** Claude calls `get_channel_stats()`. Response lists total channel count, active channel count, and for each active channel: channel name and viewer count. Example: "Channel Stats (3 active of 47 total): Active channels: BBC One — 2 viewer(s), CNN — 1 viewer(s)". If no channels are active, returns "No active channels."
- **Edge / failure tests:**
...
- **Edge / failure tests:**
  - "Show me watch history for channel ID abc-123-def" (get the ID from your channel list first) → Claude calls `get_watch_history(channel_id="abc-123-def")`; confirm filter is applied and only entries for that channel appear.
...
- **Prompt to Claude:** "Compute the smart sort order for channel 42 with stream IDs 101, 102, and 103."
- **Expected:** Claude calls `compute_stream_sort(channels=[{"channel_id": 42, "stream_ids": [101, 102, 103]}], mode="smart")`. Response lists sorted stream IDs per channel, and flags whether the order changed: "Stream Sort Results (1 channels, 1 changed): Channel 42: [102, 101, 103] (changed)". Uses a 60-second timeout.
- **Edge / failure tests:**
  - "Compute sort for channel 42 using resolution mode" → Claude passes `mode="resolution"`; verify the mode is sent in the request body and the sort results differ from smart mode.
  ...
  - "Compute sort for a channel_id that doesn't exist" → backend likely returns 404 or empty results; verify graceful handling.
- **Watch for (possible bug areas):**
...
- **Watch for (possible bug areas):**
  - **SECRET LEAKAGE RISK (HIGH):** The backend `/api/settings` response includes `discord_webhook_url` and `telegram_bot_token` in plaintext (see `backend/routers/settings.py` lines 460, 463). The MCP `get_settings()` tool only reads `s.get('discord_configured', False)` and `s.get('telegram_configured', False)` — it does NOT render these raw values. However, the full JSON response from the backend (including the raw webhook URL and bot token) passes through `ecm_client` and into the Python dict `s`. If a future refactor adds a debug dump or if the `except Exception as e` ever includes `repr(s)` in the error string, these secrets would be logged or returned. **Test:** confirm the MCP response does not contain the literal webhook URL or bot token string under any output or error path.
  - **SECRET LEAKAGE RISK (MEDIUM):** The MCP tool renders `s.get('configured', False)` — it reads the `configured` key from the backend's SettingsResponse. The backend response also includes `mcp_api_key_configured` (a boolean) — not the key itself. Confirm the raw `mcp_api_key` value is never in the backend response (SettingsResponse does not include it — only `mcp_api_key_configured`). If the response model ever changes to include the raw key, this tool would receive it unfiltered.
...
- **Edge / failure tests:**
  - "Create a backup" when the backup endpoint returns the streaming zip → **critical path**: `ecm_client.get()` calls `r.raise_for_status()` then `r.json()` — a streaming zip response will cause `r.json()` to raise `json.JSONDecodeError`. This propagates through the outer `except` and returns "Error creating backup: …" instead of the success message. This is the most likely bug in this tool for this release.
  - "Create a backup" when the ECM container has insufficient disk space → backend may return HTTP 500; verify the error detail from `_http_error` is surfaced cleanly.
  - "Create a backup" twice quickly → idempotent; the backup endpoint has no lock, so two simultaneous calls each produce their own zip. No state corruption.
- **Watch for (possible bug areas):**
...
- **Prompt to Claude:** "List the saved backups first, then delete the oldest one." (use the filename from `list_saved_backups`)
...
- **Edge / failure tests:**
  - "Delete backup ecm-backup-9999-99-99_999999.yaml" (non-existent file) → backend returns HTTP 404; `_http_error` surfaces it; tool returns "Error: DELETE /api/backup/saved/ecm-backup-9999-99-99_999999.yaml -> HTTP 404 Not Found".
  - "Delete backup ../etc/passwd" (path traversal attempt) → backend layer 1 regex `_BACKUP_FILENAME_RE` rejects it with HTTP 400 ("Invalid filename"). The MCP tool surfaces the 400 as an error. Verify the tool does NOT attempt to construct the path locally.
  - "Delete backup ecm-backup-2026-05-20_030000.yaml" twice → first call succeeds; second call returns HTTP 404.
- **Watch for (possible bug areas):**
...
- **Edge / failure tests:**
  ...
  - "Show me journal entries for category 'channels'" → Claude passes `category="channels"`; verify the filter is sent as a query param.
...
- **Watch for (possible bug areas):**
  - **SECRET LEAKAGE RISK (MEDIUM):** Journal entries record ECM activity including settings changes. The `detail` field (truncated to 80 chars) may contain partial credential values if a previous ECM version logged settings values without redaction. Example: an old `settings/update` entry might have `detail: "dispatcharr_api_key changed to sk_abc123..."`. Even with 80-char truncation, partial API keys could appear. **Test:** check whether any journal entries for the `settings` category contain credential field names with non-redacted values.
...
- **Prompt to Claude:** "Show me all unread notifications."
...
- **Edge / failure tests:**
  - "Show me notifications" when there are none → expects "No notifications."
  ...
  - "Show me notifications" when the backend returns a bare list instead of `{"notifications": [...], "total": N, "unread_count": N}` → the tool handles both shapes: `result.get("notifications", [])` falls back to using `result` directly if it's a list; `unread` defaults to 0. Verify unread count is 0 (not an error) in this case.
- **Watch for (possible bug areas):**
...
- **Edge / failure tests:**
  - "Mark all notifications as read" when there are already no unread notifications → backend marks 0 rows, returns `{"marked_read": 0}`; the read-back check sees `unread_count=0`; tool returns "All notifications marked as read." (idempotent).
  - "Mark all notifications as read" when the backend is transiently slow and the read-back fires before propagation → the WARNING fires even though the operation succeeded. This is a false positive risk; the operator should retry `list_notifications` manually to confirm.
  - "Mark all notifications as read" when the backend returns a non-dict response → `result.get("unread_count", 0) if isinstance(result, dict) else 0` — the tool handles this gracefully by defaulting to 0.
- **Watch for (possible bug areas):**
...
- **Prompt to Claude:** "Delete all notifications to clear the notification center."
...
- **Edge / failure tests:**
  - "Delete all notifications" when the inbox has both read and unread notifications → **CRITICAL BEHAVIOR BUG**: The backend `DELETE /api/notifications` defaults to `read_only=True`, which means it only deletes READ notifications. The MCP tool sends no `read_only=False` body (the endpoint contract declares no `request_fields`). Unread notifications will survive. The read-back check will show remaining notifications and trigger the WARNING — but the operator's intent ("delete all") is not fulfilled. **This is a confirmed semantic bug in the tool.**
  - "Delete all notifications" when there are no notifications → backend returns `{"deleted": 0}`; read-back sees `total=0`; tool returns "All notifications deleted." (correct).
  - "Delete all notifications" twice → first call removes read ones; second call removes any that were unread (if another tool or the UI marked them read between calls). Still affected by the `read_only=True` default.
- **Watch for (possible bug areas):**
...
- **Edge / failure tests:**
  - "Show me alert methods" when none are configured → expects "No alert methods configured."
  - "Show me alert methods" when the `alert-methods` endpoint is not available (older Dispatcharr) → outer `except` surfaces the HTTP error.
  - "Show me alert methods" when a method has no levels configured (all `notify_*` fields false) → the `level_str` is empty string and the output shows the method with no level bracket.
- **Watch for (possible bug areas):**
...
- **Prompt to Claude:** "Test alert method ID 1." (get the ID from `list_alert_methods` first)
- **Expected:** Claude calls `test_alert_method(method_id=1)`. The backend sends a test message via the configured channel (Discord webhook, email, Telegram). On success: "Test alert sent successfully. Test message dispatched." On failure: "Test alert failed: [error detail from backend]".
- **Edge / failure tests:**
  - "Test alert method 999" (non-existent ID) → backend returns HTTP 404; `_http_error` surfaces it; tool returns "Error testing alert method 999: POST /api/alert-methods/999/test -> HTTP 404 Not Found".
  - "Test alert method 1" when the Discord webhook URL is invalid → backend attempts delivery, fails, returns `{"success": false, "message": "Connection refused"}` or similar; tool returns "Test alert failed: Connection refused".
  - "Test alert method 1" when the method is disabled → behavior depends on backend — may send test anyway (ignoring enabled state) or may return `success: false`. Verify which behavior ECM implements.
  - "Test alert method 0" (zero as ID) → `method_id=0` is a valid integer but unlikely to match any real method; backend returns 404.
- **Watch for (possible bug areas):**
...
- **Edge / failure tests:**
  - "List export profiles" when none are configured → expects "No export profiles configured."
  - "List export profiles" when the backend returns a non-list response → `profiles` would be a dict; iterating `for p in profiles` on a dict iterates keys, not values. The output would list key names as profile names. This may be a latent bug if the backend ever wraps the list in an envelope.
- **Watch for (possible bug areas):**
...
- **Prompt to Claude:** "Generate the export for profile ID 2." (use the ID from `list_export_profiles` first)
- **Expected:** Claude calls `generate_export(profile_id=2)`. Backend triggers export generation (potentially long-running). Response: "Export generated for profile 2. Check ECM for download links." The tool does not return the file content — only a trigger confirmation.
- **Edge / failure tests:**
  - "Generate export for profile 999" (non-existent) → backend returns HTTP 404; tool returns "Error generating export for profile 999: …".
  - "Generate export for profile 1" when another generation for the same profile is already in progress → the backend tracks in-progress generations in `_generating: set[int]`. It may reject the concurrent request with an error. Verify the MCP tool surfaces this correctly.
  - "Generate export for profile 1" with a very large channel list → generation may take longer than the default 30-second timeout. The tool uses the client's default timeout (not the 60-second override used by `compute_stream_sort`). Verify whether a timeout produces a clean error or a hung call.
- **Watch for (possible bug areas):**
...
- **Prompt to Claude:** "List the export profiles first, then delete the one called 'Test Profile'." (use its ID)
- **Expected:** Claude calls `list_export_profiles()` to get the ID, then calls `delete_export_profile(profile_id=5)`. On success: "Export profile 5 deleted." On backend error: "Error deleting export profile 5: …".
- **Edge / failure tests:**
  - "Delete export profile 999" (non-existent) → backend returns HTTP 404; tool returns "Error deleting export profile 999: DELETE /api/export/profiles/999 -> HTTP 404 Not Found".
  - "Delete export profile 1" when a publish configuration references it → backend may return a 400/409 due to a foreign key constraint; verify the error detail is surfaced.
  - "Delete export profile 1" twice → first call succeeds; second call returns HTTP 404.
- **Watch for (possible bug areas):**
...
- **Edge / failure tests:**
  - "List cloud targets" when none are configured → expects "No cloud targets configured."
  - "List cloud targets" when the export cloud-targets endpoint returns a non-list response → same issue as `list_export_profiles`: no envelope handling. Silent incorrect output if the backend wraps the list.
- **Watch for (possible bug areas):**
  ...
  - The distinction between "cloud target" (`/api/export/cloud-targets`) and "publish config" (`/api/export/publish-configs/{config_id}/publish`) is important: `publish_export` takes a `config_id` (publish configuration), not a cloud target `id`. Operators must understand these are different resources. The MCP tools don't clarify this relationship.

---

#### `publish_export(config_id: int)` — WRITE
- **Purpose:** Publish an export to a cloud storage target.
- **Prompt to Claude:** "Publish export using publish config ID 1." (get config IDs from the ECM settings — there is no `list_publish_configs` MCP tool)
- **Expected:** Claude calls `publish_export(config_id=1)`. Backend triggers the publish pipeline for that configuration. Response: "Publish started for config 1. [message from backend if present]".
- **Edge / failure tests:**
  - "Publish export with config 999" (non-existent) → backend returns HTTP 404; `_http_error` surfaces it; tool returns "Error publishing export: …".
  - "Publish export with config 1" when the cloud target is misconfigured (wrong credentials) → the publish pipeline runs asynchronously; the MCP response may say "Publish started" even if the upload will fail. Verify whether the endpoint returns a job ID or final status.
  - "Publish export with config 1" twice concurrently → backend may queue or reject the second request.
- **Watch for (possible bug areas):**
  - **Missing prerequisite tool:** There is no `list_publish_configs` MCP tool. The operator must find the `config_id` from the ECM UI or from prior knowledge. Claude cannot help the operator discover valid config IDs through MCP alone — this is a usability gap.

...
The artifact's `journal.db` member additionally carries only an allowlist of configuration tables; every other table is dropped and the file is `VACUUM`ed. Alert methods remain, but a standard artifact redacts the ntfy topic and access token. On a restore that replaces `journal.db`, ECM preserves the destination's local ntfy topic and token only when the restored row matches that destination; otherwise the operator must re-enter them. The cred-carrying **encrypted** artifact (`include_credentials`, ADR-012 D12) is unaffected by this redaction: the identity keys join its preserve set, the URL scrub is off on that path, and its `journal.db` is a byte-for-byte copy, including ntfy topic and token. Encryption alone does not carry credentials; `include_credentials` does, and it requires a passphrase.

...
| Endpoint | Description |
|-|-|
| `GET /api/alert-methods` | List all alert methods. Admin-only when auth is enabled; the MCP service key is **admitted**, so the shipped `list_alert_methods` tool keeps working. **The response `config` is MASKED**: `password`, `bot_token`, `webhook_url`, `api_key`, and ntfy `access_token` come back as `********` for every caller, so no credential value is disclosed. Non-credential config keys are returned as stored, including ntfy `server_url` and `topic`. Until build 0096 this handler returned `config` verbatim (beads 9kwzp.10, 9kwzp.13). |
...
| `POST /api/alert-methods` | Create alert method. Admin-only when auth is enabled; the MCP service key is refused, because an alert method's `config` can carry credentials such as a webhook URL, bot token, SMTP password, or ntfy access token (bead 9kwzp.10). |
...
| `PATCH /api/alert-methods/{id}` | Update alert method. Admin-only when auth is enabled; the MCP service key is refused, because an alert method's `config` can carry notification credentials (bead 9kwzp.10). |
...
**Reads mask the credential keys (bead 9kwzp.13).** Both read routes serialize through `AlertMethod.to_dict(include_sensitive=False)`, which replaces `password`, `bot_token`, `webhook_url`, `api_key`, and `access_token` with the literal `********` (or `null` when the stored value is empty). An ntfy method therefore reads back as `{ "server_url": "https://ntfy.sh", "topic": "ecm_alerts", "access_token": "********" }`. There is **no** API path that returns the unmasked values. **Do not echo a read response back into a write**: `********` is not a write sentinel and would replace the live token.

...
Audit: accepts/rejects write `journal_entries` rows (category `event_sync`, action `review_accept`/`review_reject`); bulk discard writes `review_bulk_discard` with the exact outcome ID sets; an immediate attach writes the standard `merge_stream` entry with `after_value.match.attach_source = "review_queue"` (threshold attaches carry `"threshold"`), keeping queue-driven attaches distinguishable and covered by the journal-driven surgical unmerge.
...
| Endpoint | Description |
|-|-|
| `GET /api/alert-methods` | List all alert methods. Admin-only when auth is enabled; the MCP service key is **admitted**, so the shipped `list_alert_methods` tool keeps working. **The response `config` is MASKED**: `password`, `bot_token`, `webhook_url`, `api_key`, and ntfy `topic` and `access_token` come back as `********` for every caller, so no protected value is disclosed. Other config keys are returned as stored, including ntfy `server_url`. Until build 0096 this handler returned `config` verbatim (beads 9kwzp.10, 9kwzp.13). |
...
| `POST /api/alert-methods` | Create alert method. Admin-only when auth is enabled; the MCP service key is refused, because an alert method's `config` can carry credentials such as a webhook URL, bot token, SMTP password, or ntfy access token (bead 9kwzp.10). Creating an ntfy destination also requires a signed-in human admin on an owned instance when authentication is disabled; a genuine first-run instance with no operator identity retains anonymous setup access. |
...
| `PATCH /api/alert-methods/{id}` | Update alert method. Admin-only when auth is enabled; the MCP service key is refused, because an alert method's `config` can carry notification credentials (bead 9kwzp.10). Updating an ntfy destination has the same human-admin requirement on an owned auth-disabled instance as creating one. |
...
- **`ntfy`**: `{ "server_url": "https://ntfy.sh", "topic": "ecm_alerts", "access_token": "..." }` (`access_token` is optional)

**Reads mask protected values (bead 9kwzp.13).** Both read routes serialize through `AlertMethod.to_dict(include_sensitive=False)`, which replaces `password`, `bot_token`, `webhook_url`, `api_key`, and `access_token` with the literal `********` (or `null` when the stored value is empty). ntfy `topic` is a destination capability and is masked on the same terms, so an authenticated ntfy method reads back as `{ "server_url": "https://ntfy.sh", "topic": "********", "access_token": "********" }`. There is **no** API path that returns the unmasked values. **Do not echo a read response back into a write**: the masked topic fails required-field validation, and a masked ntfy access token is explicitly rejected rather than persisted.
...
For `method_type: "ntfy"`, `server_url` and `topic` are required. The server URL must be an absolute HTTP(S) URL with a host and valid port; embedded credentials, whitespace, backslashes, queries, and fragments are rejected. A path is allowed for self-hosted deployments. `topic` must match `^[-_A-Za-z0-9]{1,64}$`. If present, `access_token` must be a non-empty string without line breaks, must not be the API mask literal, and requires an HTTPS server URL. Tokenless HTTP remains supported for intentionally isolated self-hosted servers.
...
**`config` is REPLACED, not merged.** The partial-update semantics are per top-level field: sending `config` overwrites the whole stored blob with what you sent, so any key you omit is dropped. Required fields omitted from the replacement are rejected with `400`, but an omitted optional ntfy `access_token` is removed. To change an authenticated ntfy target, send the complete config with the real token; it cannot be recovered from a masked GET response. The Settings UI therefore creates ntfy methods but does not edit their config.
...
`POST /api/alert-methods/{id}/test` invokes the method's `test_connection()` and sends a real test message to its stored destination, including ntfy. Returns `{"success": <bool>, "message": <str>}` describing the outcome. `404` if the method doesn't exist; `200` with `success: false` if the method exists but the test failed. Failed tests are **not** modeled as `5xx`.

ntfy delivery posts the UTF-8 message body to `<server_url>/<topic>` with the alert title and priority headers, plus `Authorization: Bearer <access_token>` when configured. ECM treats only an upstream HTTP `200` as successful delivery; other statuses, redirects, timeouts, and connection failures return false without reading the upstream response body. Priority maps as follows: `info` and `success` → `3`, `warning` → `4`, and `error` → `5`. Alert-method deliveries use the existing 30-second digest buffer, so the priority of a combined ntfy message is the highest severity in that digest.
...
[
  ...
  { "type": "ntfy", "display_name": "ntfy", "required_fields": ["server_url", "topic"], "optional_fields": { "access_token": "" } }
]
```

This endpoint is the discoverable registry contract for API clients. The current Settings UI has a dedicated create-only ntfy form rather than generating forms from this response.

For scheduled-task results, ntfy participates after the existing task `send_alerts` master gate and matching `alert_on_*` severity gate pass. This change adds no `send_to_ntfy` field to scheduled-task records or APIs: enabled ntfy methods receive eligible task alerts regardless of the three legacy `send_to_email`, `send_to_discord`, and `send_to_telegram` values, then each ntfy method's own severity and applicable `alert_sources` filters are evaluated normally. TaskEngine is the single owner of scheduled `stream_probe` and `failed_stream_reprobe` completion alerts; both use the `probe_failures` source filter, the gates apply once, and ntfy does not receive a duplicate. Manual and other non-scheduled probes retain their direct completion-alert path.
...
The artifact's `journal.db` member additionally carries only an allowlist of configuration tables; every other table is dropped and the file is `VACUUM`ed. Alert methods remain, but a standard artifact redacts the ntfy topic and access token and exports no credential-verifiable destination marker. If the destination already has an authenticated ntfy target, a restore that replaces `journal.db` leaves both values unset for operator re-entry. A same-instance unauthenticated target may retain only its local topic when the restored row has the same ID, method type, and server and the local target has no token. The cred-carrying **encrypted** artifact (`include_credentials`, ADR-012 D12) is unaffected by this redaction: the identity keys join its preserve set, the URL scrub is off on that path, and its `journal.db` is a byte-for-byte copy, including ntfy topic and token. Encryption alone does not carry credentials; `include_credentials` does, and it requires a passphrase.
...
A row in `alert_methods` whose `config` does not parse as a JSON object loses its whole `config` value to the `***REDACTED***` sentinel rather than shipping unparsed. On restore, that sentinel normally preserves the destination's config instead of being stored literally. ntfy is narrower: a standard backup redacts its topic and token, so an authenticated target requires both values to be re-entered. Only a same-instance unauthenticated target may retain its local topic, and only when the restored row has the same ID, method type, and server and the local target has no token. An encrypted backup with credentials included preserves the ntfy topic and token.

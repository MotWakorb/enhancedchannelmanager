...
> **⚠️ SUPERSEDED (2026-05-21, `enhancedchannelmanager-9axgc`).** The MCP OAuth 2.1
> "Custom Connector" offering this ADR designed has been **retired** by PO
> decision. Discovery surfaced that Claude Desktop's Custom Connector is
> Anthropic-brokered and requires the MCP server to be reachable from the public
> internet — which the operator does not want. The OAuth feature was first
> **disabled** (AS endpoints unregistered → 404, the MCP RS rejecting
> OAuth/JWT-shaped Bearer tokens, the OAuth UI hidden) and then **fully removed
> from the codebase** in v0.17.3 (`enhancedchannelmanager-jir0m`): the OAuth
> router/provider/store modules, the MCP-server OAuth verify path, the OAuth
> config fields/helpers, the consent UI, and the OAuth-only tests were deleted.
> The security-critical `looks_like_jwt` no-fail-cascade guard was kept (it still
> rejects JWT-shaped Bearers so they can never reach the static-key compare). This
> ADR is **retained for decision history** only. The supported MCP authentication
> method is the static `?api_key=` path. See the Revision History entry for
> 2026-05-21 (`9axgc`) and `docs/security/threat_model_mcp_oauth.md`
> (also marked Superseded).

- **Status**: **Superseded** (retired 2026-05-21, `enhancedchannelmanager-9axgc` — MCP OAuth offering disabled; code subsequently removed from the tree in v0.17.3, `enhancedchannelmanager-jir0m`). Originally Accepted (PO sign-off 2026-05-21, AC#5) incorporating the 2026-05-21 Option A amendment (token store ECM-managed; see Revision History)
...
**Rationale — why offline verification with a shared HS256 secret, NOT a runtime callback.** The RS validates each Bearer JWT by verifying its HS256 signature against a **dedicated OAuth signing secret** (`mcp_oauth_signing_secret`) read from the shared `/config/settings.json` volume — the same volume the two containers already share for `mcp_api_key` (per `docs/architecture.md`). It does **not** call back to ECM on a per-request basis. This is a deliberate, load-bearing choice:

> **Amendment 2026-05-21 (dedicated signing secret — `buiqr.3`).** The signing secret is **dedicated to OAuth** (`settings.json` → `mcp_oauth_signing_secret`, ECM-generated on startup), **NOT** ECM's user-session `jwt.secret_key` (which lives in `auth_settings.json` and which the MCP RS never reads). Reusing the session key would have given a compromised MCP container — the most network/AI-agent-exposed component — ECM's *session-forging* key (full ECM admin takeover). Isolating the secret bounds an MCP compromise to forging **MCP-scope** tokens only (threat model SR1). This supersedes the earlier "reuse ECM's existing HS256 secret" wording in `buiqr.3`'s acceptance criteria.
...
- **One dedicated HS256 OAuth secret to protect and rotate.** Both containers read `mcp_oauth_signing_secret` from `/config/settings.json` (the MCP RS read-only). It is **separate from ECM's user-session `jwt.secret_key`** (amended 2026-05-21), so compromise of it allows forging **MCP-scope** tokens — not ECM admin sessions; rotation invalidates all live OAuth tokens (acceptable given short TTLs) without touching admin sessions. The threat model's secret-rotation / alg-confusion rows track this; protecting `settings.json` file permissions is the compensating control.
...
- **Offline verify vs runtime callback?** → Offline, **dedicated** HS256 OAuth secret (`mcp_oauth_signing_secret`, separate from the user-session key — amended 2026-05-21), no per-request callback (§1).
...
| 2026-05-21 | `enhancedchannelmanager-buiqr.3` | Amended — **dedicated OAuth signing secret** | Implementing `buiqr.3` surfaced that "sign with ECM's existing HS256 secret" meant ECM's **user-session `jwt.secret_key`** — which, under offline verification, the MCP RS must read, handing the most-exposed container ECM's session-forging key (full admin takeover on MCP compromise). **Resolution (PO-approved):** a **dedicated** `mcp_oauth_signing_secret` in `settings.json` (ECM-generated on startup), separate from `jwt.secret_key`; the RS reads only this. Bounds an MCP compromise to MCP-scope token forgery, not ECM admin sessions. Amends §1 + Consequences + Open Questions; threat model SR1/ID5 amended in lockstep; secret added to `_SETTINGS_CREDENTIAL_FIELDS`. Supersedes `buiqr.3` AC3's "auth_settings.jwt.secret_key" wording. |
| 2026-05-21 | `enhancedchannelmanager-9axgc` | **Superseded** — MCP OAuth offering retired | The PO retired the OAuth 2.1 "Custom Connector" offering after discovering that Claude Desktop's Custom Connector is **Anthropic-brokered** — Anthropic's cloud connects *out* to the MCP URL, requiring the MCP server to be reachable from the **public internet**, which the operator does not want. **Resolution (PO-approved): disable the feature + pull the docs.** The AS routers (`oauth_mcp`, `oauth_discovery`) are unregistered (`backend/routers/__init__.py`) → `/api/oauth/*` and OAuth `/.well-known/*` return 404; the startup OAuth store seed + signing-secret generation + discovery posture WARN are removed from `backend/main.py`; `GET /api/health/oauth` is removed. The MCP RS (`mcp-server/server.py`) now rejects OAuth/JWT-shaped Bearer tokens (401, no fail-cascade) and no longer serves `/.well-known/oauth-protected-resource`. The frontend Custom-Connector block, the "Active Connections" (grants) section, and the `/oauth/consent` route are removed; `OAuthConsentPage.tsx` is kept as a dormant orphan. At this point the OAuth code modules were kept dormant in-tree (NOT deleted) for reversibility (later removed — see the `jir0m` row). The static `?api_key=` MCP path is unaffected (PO-locked permanent supported method). OAuth-specific tests are skipped with reasons referencing `9axgc`; unit tests of the dormant modules stay active. `docs/runbooks/mcp-https-reverse-proxy.md` removed; `threat_model_mcp_oauth.md` marked Superseded/dormant. |
| 2026-05-24 | `enhancedchannelmanager-jir0m` | **Code removed** (Superseded ADR retained) | The PO confirmed the MCP OAuth retirement (`9axgc`) is permanent, so the dormant OAuth code was fully **deleted from the tree** in v0.17.3: the backend AS/provider/store/rate-limit/discovery modules, the MCP-server `oauth_discovery` module and the `verify_oauth_token` HS256 verifier in `oauth_rs.py`, the OAuth config fields/helpers (`mcp_oauth_signing_secret`, `oauth_allow_insecure`, `get_or_create_oauth_signing_secret`, and the MCP-server `get_signing_key*` / `get_oauth_allow_insecure` / `OAUTH_ISSUER` helpers), the frontend `OAuthConsentPage` + OAuth grant/consent API functions, and ~60 OAuth-only tests. **Preserved deliberately:** the static `?api_key=` auth path (backend `main.py` middleware, `auth/dependencies.py`, MCP-server `_handle_static_key`) and the CD1 no-fail-cascade guard `oauth_rs.looks_like_jwt` (JWT-shaped Bearers are still rejected 401 and never compared to the static key). This ADR and `threat_model_mcp_oauth.md` are retained, marked Superseded, for decision/audit history. |

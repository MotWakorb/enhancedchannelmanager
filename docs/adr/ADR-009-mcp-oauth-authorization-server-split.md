...
> internet — which the operator does not want. The OAuth feature is **disabled**:
> the AS endpoints are unregistered (404), the MCP RS rejects OAuth/JWT-shaped
> Bearer tokens, and the OAuth UI is hidden. The OAuth **code is kept dormant
> in-tree** (not deleted) so the decision is reversible. The supported MCP
> authentication method is the static `?api_key=` path. See the Revision History
> entry for 2026-05-21 (`9axgc`) and `docs/security/threat_model_mcp_oauth.md`
> (also marked Superseded/dormant).

- **Status**: **Superseded** (retired 2026-05-21, `enhancedchannelmanager-9axgc` — MCP OAuth offering disabled; code kept dormant in-tree). Originally Accepted (PO sign-off 2026-05-21, AC#5) incorporating the 2026-05-21 Option A amendment (token store ECM-managed; see Revision History)

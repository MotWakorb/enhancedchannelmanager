...
**Amendment (2026-04-24, bd-m3vej, follow-up to bd-arp3o):** the SLO-6 denominator counter at `POST /api/session-start` (added by bd-arp3o per the spike at `docs/sre/spike-slo-6-session-semantics.md`) is **unauthenticated by design** and is listed in `AUTH_EXEMPT_PATHS` in `backend/main.py`. The PO chose to open *only* the session-start endpoint, not the client-errors endpoint, so that pre-auth sessions count toward the SLO-6 denominator without exposing the richer client-error payload to unauthenticated callers. Rationale: requiring a JWT on `/api/session-start` would silently exclude login-page sessions from the SLI base, re-introducing the structural denominator bias the spike (§3.1) warned about. The `session_id` payload is a single opaque UUIDv4 with no PII; it is never logged, never returned, never persisted. The threat-model delta for opening the endpoint is bounded by the size cap (256 bytes), the strict UUIDv4 schema validation, the in-memory dedup ceiling (~daily session volume × 24h TTL), and the absence of any side effect beyond a single counter increment.

This creates a **known, intentional asymmetry**: pre-auth sessions ARE counted in the SLO-6 denominator, but pre-auth client errors are NOT counted in the numerator (because `/api/client-errors` remains JWT-required per this ADR's original posture). The resulting SLI is biased LOW — the computed error-free-session rate looks healthier than the underlying reality for any operator who experiences pre-auth bootstrap failures at material rates. Operators interpreting SLO-6 must read it with this bias in mind; alert thresholds should be set accordingly (see `docs/sre/slos.md` SLO-6 "Known measurement bias"). Closing the asymmetry by opening `/api/client-errors` to unauthenticated POSTs was explicitly considered and rejected (option B preferred over option A): the client-error payload is materially richer than the session-start payload (stack traces, message text, route, viewport, UA hash) and exposing it pre-auth would re-expand the very threat surface this ADR's allowlist (§4) was designed to bound.

...
**Observability (§D6/BD-M scope, no contract change).** `DedupHookResult` now carries the blocking `pending_merges` row id on **both** the fresh-insert and the §D5 collision branch. The pipeline result keeps three quantities apart: `pending_merges_added` counts deferred **create actions** (one stream with two create actions counts twice), `pending_merge_stream_count` counts the distinct **streams** whose channel was not created, and `pending_merge_ids` lists the distinct blocking **rows** (several streams can share one row). One shared phrase renders them in the engine completion log, the M3U-refresh task summary and — for a failed-action run — the single warning that run emits: the task engine's *Completed with Warnings* notification, or, when the run also hit the created-channel cap, the task's own combined *Capped, with errors* notification (which suppresses the engine's). Either way the metadata carries the row ids. A deferral explanation is scoped to one create attempt: a later create attempt for the same stream supersedes it, so a follow-on `assign_epg` blames the most recent cause, not a stale deferral. A run that created nothing because every stream was deferred is no longer indistinguishable from a refresh where the provider had nothing.

**Dry-run visibility is deferred to issue #1018 (maintainer decision, PR #1016).** The preview path still predicts creation for a stream the live M3U-refresh path would defer against an existing pending row. This change fixes the daily-rollover bug and does **not** claim preview/live parity; the mutation-free preview of pending-merge deferrals, with the same stream/action/row units as the live report, is owned by [#1018](https://github.com/MotWakorb/enhancedchannelmanager/issues/1018).

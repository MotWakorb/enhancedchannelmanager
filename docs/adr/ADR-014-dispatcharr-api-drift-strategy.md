...
- 97 Dispatcharr call sites in `backend/dispatcharr_client.py`, over **58 distinct URL templates** (92 distinct `(method, template)` pairs) — this ADR's own `get_version()` addition takes that to 59; 27 of those templates are DBAS-restore-critical.
...
3. **Coverage cannot silently shrink, and every unreadable httpx seam is visible.** Extraction walks *every* class and top-level function in the module, so moving methods onto a mixin or adding a second request-issuing class does not drop them. Three seam shapes whose URL the extractor cannot read — a non-verb call on `self._client`, constructing an httpx client, and binding `self._client` to a local name — fail a test until a human registers them as reviewed plumbing, keyed per `Class.method` so a same-named method elsewhere cannot inherit someone else's review. (All four evasions were demonstrated against the first implementation in review; each left the sweep green while dropping real coverage.)
...
- **Password mode reuses the access token the login just issued** and never triggers a second one — Dispatcharr rate-limits login at 3/min per IP, so an advisory that re-authenticated could burn the operator's budget and fail their *next* real test. With no reusable token the probe is skipped entirely.

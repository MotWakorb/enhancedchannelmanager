...
> **Audience:** Operator who set a Channel Sort on a rule and expected it to
> renumber their channels alphabetically, but the channel numbers didn't
> change.
...
all streams have been processed — regardless of which rule or action created
each channel, and independent of each Create Channel action's Channel Number
setting. This is the same sort used by the manual **Sort & Renumber** tool in

...
| [`runaway-safety-cap.md`](runaway-safety-cap.md) | The per-run channel cap (the GH #473 safety valve): why a run gets "capped", that auto-creation is idempotent so you can just re-run, and how to view/raise/disable the cap (and its sibling log-entries cap) from **Settings → Auto Creation** (admin-only). |

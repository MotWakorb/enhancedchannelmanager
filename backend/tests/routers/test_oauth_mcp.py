...
# MCP OAuth offering RETIRED (bd-9axgc). The OAuth AS routers are no longer
# registered (routers/__init__.py), so /api/oauth/* returns 404. Every test in
# this module exercises those now-disabled HTTP endpoints through the app, so
# the whole module is skipped. Re-enable when MCP OAuth is re-offered.

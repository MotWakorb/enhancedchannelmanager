...
Tests: GET /api/health, GET /api/health/ready, GET /api/health/oauth,
...
class TestOAuthStoreDiagnostics:
    ...
    Verifies the three diagnostic codes that ECM surfaces for its owned
    OAuth state store (/config/mcp_oauth.db):

      - token_store_unreadable — DB file missing or unreadable (ENOENT/EACCES).
      - token_store_corrupt    — SQLite integrity_check reports errors.
      - client_registry_missing — oauth_clients table is empty / unseeded.
    """

    @pytest.mark.asyncio
    async def test_oauth_store_ok_when_healthy(self, async_client):
        """All three codes absent when the store is present, intact, and seeded."""
        with patch(
            "routers.health.get_oauth_store_status",
            return_value={
            ...
            response = await async_client.get("/api/health/oauth")
        assert response.status_code == 200
        data = response.json()
        assert data["status"] == "ok"
        assert data["checks"]["token_store_unreadable"]["status"] == "ok"
        assert data["checks"]["token_store_corrupt"]["status"] == "ok"
        assert data["checks"]["client_registry_missing"]["status"] == "ok"

    @pytest.mark.asyncio
    async def test_oauth_store_unreadable_when_missing(self, async_client):
        """token_store_unreadable triggered when DB file does not exist."""
        with patch(
            "routers.health.get_oauth_store_status",
            return_value={
                "token_store_unreadable": "token_store_unreadable",
                "token_store_corrupt": None,
                "client_registry_missing": None,
            },
        ):
        ...
        check = data["checks"]["token_store_unreadable"]
        assert check["status"] == "fail"
        assert check["code"] == "token_store_unreadable"
        assert "setup_hint" in check
        assert data["checks"]["token_store_corrupt"]["status"] == "ok"
        assert data["checks"]["client_registry_missing"]["status"] == "ok"

    @pytest.mark.asyncio
    async def test_oauth_store_corrupt_when_integrity_fails(self, async_client):
        """token_store_corrupt triggered when SQLite integrity_check reports errors."""
        with patch(
            "routers.health.get_oauth_store_status",
            return_value={
                "token_store_unreadable": None,
                "token_store_corrupt": "token_store_corrupt",
                "client_registry_missing": None,
            },
        ):
            response = await async_client.get("/api/health/oauth")
        ...
        data = response.json()
        assert data["status"] == "degraded"
        check = data["checks"]["token_store_corrupt"]
        assert check["status"] == "fail"
        assert check["code"] == "token_store_corrupt"
        assert "setup_hint" in check

    @pytest.mark.asyncio
    async def test_client_registry_missing_when_empty(self, async_client):
        """client_registry_missing triggered when oauth_clients table is empty."""
        with patch(
            "routers.health.get_oauth_store_status",
            return_value={
                "token_store_unreadable": None,
                "token_store_corrupt": None,
                "client_registry_missing": "client_registry_missing",
            },
        ):
            response = await async_client.get("/api/health/oauth")
        assert response.status_code == 200
        data = response.json()
        assert data["status"] == "degraded"
        check = data["checks"]["client_registry_missing"]
        assert check["status"] == "fail"
        assert check["code"] == "client_registry_missing"
        assert "setup_hint" in check

    @pytest.mark.asyncio
    async def test_multiple_failures_all_reported(self, async_client):
        """All failing codes are reported independently when multiple failures occur."""
        with patch(
            "routers.health.get_oauth_store_status",
            return_value={
                "token_store_unreadable": "token_store_unreadable",
        ...
        data = response.json()
        assert data["status"] == "degraded"
        for code in ("token_store_unreadable", "token_store_corrupt", "client_registry_missing"):
            assert data["checks"][code]["status"] == "fail"

    @pytest.mark.asyncio
    async def test_no_secrets_in_response(self, async_client):
        """The response must not contain any token values, hashes, or DB paths."""
        with patch(
            "routers.health.get_oauth_store_status",
            return_value={
                "token_store_unreadable": None,
            ...
            response = await async_client.get("/api/health/oauth")
        raw = response.text
        # The response must not contain any embedded token-like strings.
        assert "mcp_oauth.db" not in raw
        assert "secret" not in raw.lower().replace("setup_hint", "")


# ============================================================================
# Unit tests for get_oauth_store_status() — the pure diagnostic helper
# ============================================================================
class TestGetOAuthStoreStatus:
    """Unit tests for auth.oauth_store.get_oauth_store_status().

    These tests exercise the actual SQLite open / integrity-check logic
    directly, without going through the HTTP layer.
    """

    def test_returns_all_none_for_healthy_store(self, tmp_path):
        """A seeded, intact store returns no failing codes."""
        from auth.oauth_store import OAuthStore, get_oauth_store_status
        ...
        result = get_oauth_store_status(db_path)
        assert result["token_store_unreadable"] is None
        assert result["token_store_corrupt"] is None
        assert result["client_registry_missing"] is None

    def test_token_store_unreadable_when_file_absent(self, tmp_path):
        """Returns token_store_unreadable when the DB file does not exist."""
        from auth.oauth_store import get_oauth_store_status

        db_path = tmp_path / "nonexistent.db"
        result = get_oauth_store_status(db_path)
        assert result["token_store_unreadable"] == "token_store_unreadable"
        assert result["token_store_corrupt"] is None
        assert result["client_registry_missing"] is None

    def test_token_store_corrupt_when_db_broken(self, tmp_path):
        """Returns token_store_corrupt when the file exists but is not valid SQLite."""
        db_path = tmp_path / "corrupt.db"
        db_path.write_bytes(b"this is not sqlite data at all\x00\x01\x02")

        from auth.oauth_store import get_oauth_store_status

        result = get_oauth_store_status(db_path)
        assert result["token_store_corrupt"] == "token_store_corrupt"

    def test_client_registry_missing_when_table_empty(self, tmp_path):
        """Returns client_registry_missing when oauth_clients table has no rows."""
        from auth.oauth_store import OAuthStore, get_oauth_store_status

        db_path = tmp_path / "empty_clients.db"
        store = OAuthStore(db_path)
        store.init_schema()
        # Deliberately do NOT seed clients.
        store.close()

        result = get_oauth_store_status(db_path)
        assert result["token_store_unreadable"] is None
        assert result["token_store_corrupt"] is None
        assert result["client_registry_missing"] == "client_registry_missing"

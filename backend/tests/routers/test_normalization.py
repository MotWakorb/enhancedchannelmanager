...
class TestDeleteGroup:
    ...
    @pytest.mark.asyncio
    async def test_strips_group_ref_from_auto_creation_rules(self, async_client, test_session):
        ...
        rule = ChannelPipelineRule(
            ...
            conditions="[]",
            actions="[]",
            normalization_group_ids=json.dumps([doomed.id, kept.id]),
        )
        ...
        response = await async_client.delete(f"/api/normalization/groups/{doomed_id}")
        assert response.status_code == 200
    ...
    async def test_delete_leaves_unrelated_rules_untouched(self, async_client, test_session):
        ...
        rule = ChannelPipelineRule(
            ...
            conditions="[]",
            actions="[]",
            normalization_group_ids=json.dumps([other.id]),
        )
        test_session.add(rule)
        test_session.commit()
        rule_id = rule.id

        response = await async_client.delete(f"/api/normalization/groups/{doomed.id}")
        assert response.status_code == 200

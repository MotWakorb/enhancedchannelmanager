...
class TestWalCheckpointTruncate:
    ...
    def _build_engine_with_dirty_wal(self, tmp_path):
        ...
        # Wrapped in an outer try so reader.close() runs even if the
        # writer block raises (the inner finally would otherwise close
        # writer-only and leak reader until GC).
        ...
        try:
            ...
            try:
                ...
                # Close writer FIRST while reader is still pinning the
                # WAL — otherwise SQLite triggers an implicit checkpoint
                # on the last-connection-close and our "dirty WAL" goes
                # away before the sanity check. Reader closes in the
                # outer finally below.
                writer.close()
            ...
            # Always close the reader, even if the writer block raised
            # — prevents the leak the inner-finally-only version had.
            reader.close()
    ...
    def test_busy_checkpoint_logs_warning_not_info(self, tmp_path, monkeypatch, caplog):
        """When PRAGMA returns ``busy=1`` the checkpoint was incomplete.

        The PRAGMA wal_checkpoint(TRUNCATE) call returns ``(busy, log,
        checkpointed)``. ``busy=1`` means SQLite could not acquire the
        exclusive WAL lock (some other connection is still holding a
        reader open) and the WAL was NOT fully truncated. That's the
        GH #274 disease vector — a pinned reader leaves the WAL bloated
        even though the PRAGMA "ran". The helper MUST surface that as
        WARN so the operator can investigate; an INFO log would let the
        same pathology that motivated bd-ej995 hide in the noise.
        """
        import logging
        monkeypatch.setattr(database, "JOURNAL_DB_FILE", tmp_path / "journal.db")

        # Mock the engine.connect() context manager and its execute(...)
        # so the PRAGMA returns (1, 0, 0) — busy. We use MagicMock for
        # the context-manager protocol; the .fetchone() call is what the
        ...
        with caplog.at_level(logging.INFO, logger=database.logger.name):
            database._wal_checkpoint_truncate(mock_engine)

        # The "incomplete" / "busy" signal must show up at WARNING level
        # — INFO would let the pinned-reader pathology hide.
        warn_records = [
        ...
        # And explicitly: it must NOT have been logged as INFO.
        info_busy_records = [
            r for r in caplog.records
            if r.levelno == logging.INFO and "WAL checkpoint" in r.message
        ]
        assert not info_busy_records, (
            "busy checkpoint must NOT log at INFO — that would hide the bloat: "
            f"got {[r.message for r in info_busy_records]}"
        )

...
class ActionExecutor:
    ...
    async def _execute_unassign_profile(
        ...
        description = (
            f"Would remove stream profile {current_profile_id} from channel"
            if exec_ctx.dry_run
            else f"Removed stream profile {current_profile_id} from channel"
        )
        if exec_ctx.dry_run:
    ...
    async def _execute_unassign_channel_profile(
        ...
        if target == "all":
            ...
            remove_ids = list(self._all_profile_ids)
        else:
            remove_ids = list(selected_ids)
    ...
    async def apply_channel_profile_to_channels(
        ...
        """Apply a channel-profile action to an EXPLICIT set of channel ids.

        This is the event_sync execution path (GH #720 / y3m6o.1 Finding 4).
        ...
        try:
            for channel_id in ordered_ids:
                exec_ctx.current_channel_id = channel_id
                if action.type == ActionType.UNASSIGN_CHANNEL_PROFILE.value:
                    result = await self._execute_unassign_channel_profile(
                        action, exec_ctx, rule_id=rule_id
                    )
                else:

...
def _find_matching_sessions(
    ...
    * **Tier 3** — RapidFuzz ``token_set_ratio`` on ``ecm_stream_name``
    ...
    # it as a Tier-2 candidate. Numeric-only prefixes are accepted (so
    # word-prefixed channel names cannot collide with arbitrary session
    # ``channel_number`` strings). When the caller DOES supply an
    # explicit ``ecm_channel_number``, that value wins — the parser
    # never overrides an explicit signal.
...
def _parse_numeric_pipe_prefix(ecm_channel_name: str | None) -> str:
    ...
    # ``str.replace(".", "", 1)`` collapses a single dot so a
    # well-formed sub-channel like "2.1" → "21" passes ``.isdigit()``;
    # multi-dot or alphabetic prefixes ("WI", "news.1.2") fall through.
...
def _coerce_tier2_number(
    ...
    it is present (and non-empty after string-coerce + strip), the
    resolver compares only that value. When it is ``None``, the helper
    falls back to ``_parse_numeric_pipe_prefix(ecm_channel_name)`` so a
    ...
    (both inputs absent or the name's prefix is non-numeric); callers
    treat ``""`` as "skip Tier-2".
    """

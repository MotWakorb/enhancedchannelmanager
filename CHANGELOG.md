...
### Fixed
- Scheduled task email alerts now deliver via the SMTP Email alert method (recipients in `to_emails`) instead of being a no-op, and Notification Settings now exposes an "Email Alert Recipients" field to configure those recipients in the UI (`bd-enhancedchannelmanager-bq27w`, `bd-enhancedchannelmanager-lfmuw`).

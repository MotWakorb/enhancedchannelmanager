...
export function useChannelPipelineRules(
  ...
  const duplicateRule = useCallback(async (id: number): Promise<ChannelPipelineRule> => {
    ...
    const duplicateData: CreateRuleData = {
      ...
      match_scope_target_group: originalRule.match_scope_target_group,
      match_scope_group_id: originalRule.match_scope_group_id,
      allow_manual_channel_merge: originalRule.allow_manual_channel_merge,
      fold_match_key: originalRule.fold_match_key,
      required_provider_ids: originalRule.required_provider_ids,

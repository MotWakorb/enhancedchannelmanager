...
export function generateCommand(config: FFMPEGBuilderState): GeneratedCommand {
    ...
    flags.push({
        text: `-i ${inputPath}`, category: 'input', explanation: inputPath === '{streamUrl}' || inputPath.startsWith('http') || inputPath.startsWith('rtmp')
    ...
    if (vc) {
        ...
        if (vc.scThreshold !== undefined) {
            ...
            flags.push({
                text: `-sc_threshold ${vc.scThreshold}`, category: 'video', explanation: vc.scThreshold === 0
...
export type { GeneratedCommand, CommandFlag };
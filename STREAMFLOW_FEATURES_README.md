# StreamFlow Features für ECM

## Übersicht

Diese Features wurden aus StreamFlow-mod nach ECM portiert, um erweiterte Stream-Management-Funktionen bereitzustellen:
...
```bash
GET /api/streamflow-features/config
```
...
```bash
PUT /api/streamflow-features/provider-diversification
...
```bash
PUT /api/streamflow-features/account-stream-limits
Content-Type: application/json
...
```bash
PUT /api/streamflow-features/m3u-priority
Content-Type: application/json
...
- `ECM/backend/streamflow_features_config.py` - Konfigurations-Management
- `ECM/backend/routers/streamflow_features.py` - API-Endpunkte

### Konfigurationsdatei:
- `/config/streamflow_features.json` - Persistente Konfiguration
...
```python
# Import StreamFlow features router
from routers.streamflow_features import router as streamflow_features_router
app.include_router(streamflow_features_router)
```
...
## 📊 Vergleich: StreamFlow vs ECM

| Feature | StreamFlow | ECM (portiert) |
|---------|-----------|----------------|
...
Diese Features wurden aus StreamFlow-mod portiert und unterliegen der gleichen Lizenz wie ECM.

**Original StreamFlow:** https://github.com/krinkuto11/streamflow
